 ...
 * $Author: lrichardson $
 * $Locker:  $
 * $Date: 2024/10/24 21:23:25 $
 * $Id: libl2.c,v 1.50 2024/10/24 21:23:25 lrichardson Exp $
 * $Revision: 1.50 $
 * $State: Exp $
 */
...
#define	MAX_NUM_STATUS_MSGS		200
#define RDA_PRF_TABLES_MSG              32
...
typedef ORDA_prf_tables_data_t    libL2_prf_tables_t;
static libL2_prf_tables_t  *L2_prf_tables = NULL;
static libL2_clutter_map_t *L2_clutter = NULL;
static libL2_bypass_map_t  *L2_bypass = NULL;
static libL2_status_t       L2_status_msgs[MAX_NUM_STATUS_MSGS];
...
/************************************************************************
 Description: Return pointer to RDA prf tables data in metadata.
 ************************************************************************/

libL2_prf_tables_t *libL2_prf_tables_msg()
{
  static char *fx = "libL2_prf_tables_msgs";

  if( L2_prf_tables == NULL )
  {
    Error_code = LIBL2_NO_PRF_TABLE_MSG;
    sprintf( Error_msg, "%s: No RDA PRF Table Msgs in %s (%d)", fx, L2_file_name_buf, Error_code );
    libL2_debug( "%s", Error_msg );
    libL2_debug( "%s: L2_prf_tables is NULL", fx );
    return NULL;
  }

  return L2_prf_tables;
}
...
static int Init_variables( char *l2_filename )
{
  ...
  if( L2_prf_tables != NULL )
  {
    libL2_debug( "%s: L2_prf_tables previously defined", fx );
    free( L2_prf_tables );
    L2_prf_tables = NULL;
  }
  if( ( L2_prf_tables = malloc( sizeof( libL2_prf_tables_t ) ) ) == NULL )
  {
    Error_code = LIBL2_MALLOC_FAILED;
    sprintf( Error_msg, "%s: failed malloc for L2_prf_tables (%d)", fx, Error_code );
    libL2_debug( "%s", Error_msg );
    return Error_code;
  }
...
static int Parse_L2_buf()
{
  ...
  if( ( ret = Is_start_of_volume_file() ) != LIBL2_NO_ERROR )
  {
    ...
    printf( "%s: Is_start_of_volume_file returns %d", fx, ret );
    return ret;
  }
  else if( ( ret = Parse_meta_data() ) != LIBL2_NO_ERROR )
  {
    ...
    printf( "%s: Parse_meta_data returns %d", fx, ret );
    return ret;
  }
  else if( ( ret = Parse_base_data() ) != LIBL2_NO_ERROR )
  {
    ...
    printf( "%s: Parse_base_data returns %d", fx, ret );
    return ret;
  }
...
#define RDA_PRF_TABLES_NUM_SEGS 1
#define RDA_PRF_TABLES_MAX_LEN  RDA_PRF_TABLES_NUM_SEGS*METADATA_MSG_SIZE
...
#define	NUM_METADATA_MSGS	( RDA_PRF_TABLES_NUM_SEGS + RDA_ADAPT_NUM_SEGS + RDA_CLUTTER_NUM_SEGS + RDA_BYPASS_NUM_SEGS + RDA_VCP_NUM_SEGS + RDA_PMD_NUM_SEGS + RDA_STATUS_NUM_SEGS )
...
static int Parse_meta_data()
{
  ...
  char  msg32buf[RDA_PRF_TABLES_MAX_LEN];
  int   msg32_index = 0;
  ...
  memset( msg13buf, 0, RDA_BYPASS_MAX_LEN );
  memset( msg32buf, 0, RDA_PRF_TABLES_MAX_LEN );
  ...
  while( buf_offset < MAX_METADATA_SIZE )
  {
    ...
    Channel = msg_header->rda_channel;
    ...
    else if( type == RDA_PRF_TABLES_MSG )
    {
      libL2_debug( "%s: MSG 32: SIZE: %d INDEX: %d SEGMENT: %d OF %d", fx,
                   msg_size, msg32_index, msg_segment_number, number_of_msg_segments );
      memcpy( (char *)(msg32buf+msg32_index),
              (char *)msg_header+copy_offset, copy_size );
      msg32_index += copy_size;
      msg_header = (libL2_msg_hdr_t *)msg32buf;
      msg_header->size = msg32_index;
    ...
    else if( type == RDA_STATUS_MSG )
    {
      ...
      /* RDA Status is not a multi-segmented message.  copy_size is
         also defined in bytes so we don't want to modify the header.
         Not sure why the following code existed in the first place
...
//    msg_header->size = msg2_index;
  ...
  libL2_debug( "%s: MSG SIZES 32: %d 18: %d 15: %d 13: %d 5: %d 3: %d 2: %d", fx,
               msg32_index, msg18_index, msg15_index, msg13_index,
  ...
  if( has_prf ){
     ...
     libL2_debug( "%s: byte swap RDA PRF TABLES msg32", fx );
     UMC_RDAtoRPG_message_convert_to_internal( RDA_PRF_TABLES_MSG, (char *)msg32buf );
  ...
  if( has_prf ){
     memcpy( (char *) &L2_prf_tables[0], &msg32buf[sizeof(libL2_msg_hdr_t)], sizeof( libL2_prf_tables_t ) );
     if( Debug_flag ){ libL2_print_rda_prf_tables(); }
  }
...
{
  static char *fx = "Add_status_msg";
  libL2_status_t *sm = (libL2_status_t *) status_msg;
  int size_of_msg = sm->msg_hdr.size*sizeof(short);

  if( L2_num_status_msgs >= MAX_NUM_STATUS_MSGS ){

      libL2_debug( "%s: Too Many Status Messages %d", fx, L2_num_status_msgs ); 
      return LIBL2_NO_STATUS_MSGS;

  }

  /* In Build 18, the size of the status message increased in size.  
     Need to extract the size from the message itself to know 
     how large it is. */
  memcpy( &L2_status_msgs[L2_num_status_msgs], &status_msg[0], size_of_msg );
  L2_num_status_msgs++;
...
int libL2_print_rda_adapt()
{
  ...
  printf( "  NAME OF ADAPTATION DATA FILE:                 %s\n", L2_adapt->adap_file_name );
  printf( "  FORMAT OF ADAPTATION DATA FILE:               %s\n", L2_adapt->adap_format );
  printf( "  REVISION NUMBER OF ADAPTATION DATA FILE:      %s\n", L2_adapt->adap_revision );
  printf( "  LAST MODIFIED DATE ADAPTATION DATA FILE:      %s\n", L2_adapt->adap_date );
  printf( "  LAST MODIFIED TIME ADAPTATION DATA FILE:      %s\n", L2_adapt->adap_time );
  printf( "  ANGLE OF LOWER PRE-LIMIT SWITCH:              %4.2f\n", L2_adapt->lower_pre_limit );
  printf( "  LATENCY OF AZIMUTH ENCODER MEASUREMENT:       %6.4f sec\n", L2_adapt->az_lat );
  printf( "  ANGLE OF THE UPPER PRE-LIMIT SWITCH:          %4.2f\n", L2_adapt->upper_pre_limit );
  printf( "  LATENCY OF ELEVATION ENCODER MEASUREMENT:     %6.4f sec\n", L2_adapt->el_lat );
  printf( "  PEDESTAL PARK POSITION IN AZIMUTH:            %6.2f deg\n", L2_adapt->parkaz );
  printf( "  PEDESTAL PARK POSITION IN ELEVATION:          %5.2f deg\n", L2_adapt->parkel );
  for( i = 0; i <= 10; i++ )
  {
    printf( "  GENERATOR FUEL LEVEL HT/CAP CONV (%2d):     %5.1f %%\n", i, L2_adapt->a_fuel_conv[i] );
  }
  printf( "  MINIMUM EQUIPMENT SHELTER ALARM TEMP:         %4.1f C\n", L2_adapt->a_min_shelter_temp );
  printf( "  MAXIMUM EQUIPMENT SHELTER ALARM TEMP:         %4.1f C\n", L2_adapt->a_max_shelter_temp );
  printf( "  MIN A/C DISCHARGE AIR TEMP DIFFERENTIAL:      %4.1f C\n", L2_adapt->a_min_shelter_ac_temp_diff );
  printf( "  MAX TRANSMITTER LEAVING AIR ALARM TEMP:       %4.1f C\n", L2_adapt->a_max_xmtr_air_temp );
  printf( "  MAXIMUM RADOME ALARM TEMPERATURE:             %4.1f C\n", L2_adapt->a_max_rad_temp );
  printf( "  MAX RADOME MINUS AMBIENT TEMP DIFFERENCE:     %4.1f C\n", L2_adapt->a_max_rad_temp_rise );
  printf( "  ANGLE OF LOWER DEAD LIMIT SWITCH:             %4.1f %%\n", L2_adapt->lower_dead_limit );
  printf( "  ANGLE OF UPPER DEAD LIMIT SWITCH:             %4.1f %%\n", L2_adapt->upper_dead_limit );
  printf( "  MINIMUM GENERATOR SHELTER ALARM TEMP:         %4.1f C\n", L2_adapt->a_min_gen_room_temp );
  printf( "  MAXIMUM GENERATOR SHELTER ALARM TEMP:         %4.1f C\n", L2_adapt->a_max_gen_room_temp );
  printf( "  SPIP +5V POWER SUPPLY TOLERANCE:              %4.1f %%\n", L2_adapt->spip_5V_reg_lim );
  printf( "  SPIP +/-15V POWER SUPPLY TOLERANCE:           %4.1f %%\n", L2_adapt->spip_15V_reg_lim );
  printf( "  RPG CO-LOCATED:                               %s\n", L2_adapt->rpg_co_located );
  printf( "  TRANSMITTER SPECTRUM FILTER INSTALLED:        %s\n", L2_adapt->spec_filter_installed );
  printf( "  TRANSITION POWER SOURCE INSTALLED:            %s\n", L2_adapt->tps_installed );
  printf( "  FAA RMS INSTALLED:                            %s\n", L2_adapt->rms_installed );
  printf( "  PERFORMANCE TEST INTERVAL:                    %2d hr\n", L2_adapt->a_hvdl_tst_int );
  printf( "  RPG LOOP TEST INTERVAL:                       %2d min\n", L2_adapt->a_rpg_lt_int );
  printf( "  REQUIRED INTERVAL TIME STABLE UTILITY PWR:    %2d min\n", L2_adapt->a_min_stab_util_pwr_time );
  printf( "  MAX GENERATOR AUTOMATIC EXERCISE INTERVAL:    %3d hr\n", L2_adapt->a_gen_auto_exer_interval );
  printf( "  RECD SWITCH TO UTILITY POWER TIME INTERVAL:   %2d min\n", L2_adapt->a_util_pwr_sw_req_interval );
  printf( "  LOW FUEL TANK WARNING LEVEL:                  %5.1f %%\n", L2_adapt->a_low_fuel_level );
  printf( "  CONFIGURATION CHANNEL NUMBER:                 %1d\n", L2_adapt->config_chan_number );
/*
  printf( "  SPARE-220:                                    %d\n", L2_adapt->spare_220 );
*/
  ...
  if( i == 1 )
  {
    printf( "  REDUNDANT CHANNEL CONFIGURATION:              SINGLE (%d)\n", i );
  }
  else if( i == 2 )
  {
    printf( "  REDUNDANT CHANNEL CONFIGURATION:              FAA (%d)\n", i );
  }
  else if( i == 3 )
  {
    printf( "  REDUNDANT CHANNEL CONFIGURATION:              NWS-R (%d)\n", i );
  }
  else
  {
    printf( "  REDUNDANT CHANNEL CONFIGURATION:              UNKNOWN (%d)\n", i );
  }

  for( i = 0; i < 104; i++ )
  {
    printf( "  TEST SIGNAL ATTENUATOR INSERT LOSS-%3d:       %7.2f dB\n", i, L2_adapt->atten_table[i] );
  }
  printf( "  PATH LOSS-VERTICAL IF HELIAX TO 4AT16 (7):    %6.2f dB\n", L2_adapt->path_losses[6] );
  printf( "  PATH LOSS-2A9A9 RF DELAY LINE (13):           %6.2f dB\n", L2_adapt->path_losses[12] );
  printf( "  PATH LOSS-HORIZONTAL IF HELIAX TO 4AT17 (28): %6.2f dB\n", L2_adapt->path_losses[27] );
  printf( "  PATH LOSS-A5 ELEVATION ROTARY JOINT (30):     %6.2f dB\n", L2_adapt->path_losses[29] );
  printf( "  PATH LOSS-A5 ELEV ROTARY JOINT TO ANT (31):   %6.2f dB\n", L2_adapt->path_losses[30] );
  printf( "  PATH LOSS-WG02 HARMONIC FILTER (32):          %6.2f dB\n", L2_adapt->path_losses[31] );
  printf( "  PATH LOSS-WAVEGUIDE KLYSTRON TO SWITCH (33):  %6.2f dB\n", L2_adapt->path_losses[32] );
  printf( "  PATH LOSS-2A1A4 WG CHANNEL AZ ROTARY (34):    %6.2f dB\n", L2_adapt->path_losses[33] );
  printf( "  PATH LOSS-WG06 SPECTRUM FILTER (35):          %6.2f dB\n", L2_adapt->path_losses[34] );
  printf( "  PATH LOSS-WG SWITCH TO AZ ROT JOINT (37):     %6.2f dB\n", L2_adapt->path_losses[36] );
  printf( "  PATH LOSS-WAVEGUIDE SWITCH (38):              %6.2f dB\n", L2_adapt->path_losses[37] );
  printf( "  PATH LOSS-WG04 CIRCULATOR (39):               %6.2f dB\n", L2_adapt->path_losses[38] );
  printf( "  PATH LOSS-A6 ARC DETECTOR (40):               %6.2f dB\n", L2_adapt->path_losses[39] );
  printf( "  PATH LOSS-1DC1 XMTR COUPLER STRAIGHT (41):    %6.2f dB\n", L2_adapt->path_losses[40] );
  printf( "  PATH LOSS-1DC1 XMTR COUPLER COUPLING (42):    %6.2f dB\n", L2_adapt->path_losses[41] );
  printf( "  PATH LOSS-A33 PAD (43):                       %6.2f dB\n", L2_adapt->path_losses[42] );
  printf( "  PATH LOSS-COAX XMTR RF SAMPLE TO A33 (44):    %6.2f dB\n", L2_adapt->path_losses[43] );
  printf( "  PATH LOSS-A20J1_4 POWER SPLITTER (45):        %6.2f dB\n", L2_adapt->path_losses[44] );
  printf( "  PATH LOSS-A20J1_3 POWER SPLITTER (46):        %6.2f dB\n", L2_adapt->path_losses[45] );
  printf( "  PATH LOSS-A20J1_2 POWER SPLITTER (47):        %6.2f dB\n", L2_adapt->path_losses[46] );
  printf( "  RF PALLET HORIZ COUPLER TEST SIGNAL LOSS:     %6.2f dB\n", L2_adapt->path_losses[47] );
  printf( "  RF PALLET VERT COUPLER XMTR SIGNAL LOSS:      %6.2f dB\n", L2_adapt->path_losses[48] );
  printf( "  PATH LOSS-WG AZ JOINT TO ELEV JOINT (50):     %6.2f dB\n", L2_adapt->path_losses[49] );
  printf( "  AME TEST SIGNAL BIAS:                         %6.2f dB\n", L2_adapt->path_losses[50] );
  printf( "  PATH LOSS-1AT4 TRANSMITTER COUPLE PAD (52):   %6.2f dB\n", L2_adapt->path_losses[51] );
  printf( "  RF PALLET VERT COUPLER TEST SIGNAL LOSS:      %6.2f dB\n", L2_adapt->path_losses[52] );
  printf( "  POWER SENSE CALIBRATION OFFSET BIAS:          %6.2f dB\n", L2_adapt->path_losses[55] );
  printf( "  AME NOISE SOURCE VERT EXCESS NOISE RATIO:     %6.2f dB\n", L2_adapt->path_losses[56] );
  printf( "  PATH LOSS-4AT17 ATTENUATOR (58):              %6.2f dB\n", L2_adapt->path_losses[57] );
  printf( "  PATH LOSS-IFDR IF ANTI_ALIAS FILTER (59):     %6.2f dB\n", L2_adapt->path_losses[58] );
  printf( "  PATH LOSS-A20J1_5 POWER SPLITTER (60):        %6.2f dB\n", L2_adapt->path_losses[59] );
  printf( "  PATH LOSS-AT5 50dB ATTENUATOR (61):           %6.2f dB\n", L2_adapt->path_losses[60] );
  printf( "  PATH LOSS-A39 RF_IF BURST MIXER TO AR1 (63):  %6.2f dB\n", L2_adapt->path_losses[62] );
  printf( "  PATH LOSS-AR1 BURST IF AMPLIFIER (64):        %6.2f dB\n", L2_adapt->path_losses[63] );
  printf( "  PATH LOSS-IFDR BURST ANTI_ALIAS FILTER (65):  %6.2f dB\n", L2_adapt->path_losses[64] );
  printf( "  PATH LOSS-DC3J1_3 6dB COUPLER, THROUGH (66):  %6.2f dB\n", L2_adapt->path_losses[65] );
  printf( "  PATH LOSS-4DC3J1 TO 4A39 L (67):              %6.2f dB\n", L2_adapt->path_losses[66] );
  printf( "  PATH LOSS-AT2+AT3 26dB COHO ATTENUATOR (68):  %6.2f dB\n", L2_adapt->path_losses[67] );
 

  printf( "  NONCONTROLLING CHANNEL CALIB DIFFERENCE:      %4.2f dB\n", L2_adapt->chan_cal_diff );
  printf( "  AME VERTCAL TEST SIGNAL POWER:                %5.2f dBm\n", L2_adapt->v_ts_cw );
  for( i = 0; i < 13; i++ )
  {
    printf( "  HORIZ RECEIVER NOISE NORMALIZATION-%2d:         %5.3f\n", i, L2_adapt->rnscale[i] );
  }
  for( i = 0; i < 13; i++ )
  {
    printf( "  TWO-WAY ATMOSPHERIC LOSS-%2d:                 %7.4f dB/km\n", i, L2_adapt->atmos[i] );
  }
  for( i = 0; i < 12; i++ )
  {
    printf( "  BYPASS MAP GENERATION ELEV ANGLE-%2d:         %6.3f deg\n", i, L2_adapt->el_index[i] );
  }
  printf( "  TRANSMITTER FREQUENCY:                        %4d Mhz\n", L2_adapt->tfreq_mhz );
  printf( "  POINT CLUTTER SUPPRESSION THRESHOLD:          %4.1f dB\n", L2_adapt->base_data_tcn );
  printf( "  RANGE UNFOLDING OVERLAY THRESHOLD (TOVER):    %4.1f dB\n", L2_adapt->refl_data_tover );
  printf( "  HORIZ TARGET SYSTEM CALIB (dBZ0) LONG PULSE:  %6.2f dBZ\n", L2_adapt->tar_h_dbz0_inc_lp );
  printf( "  VERT TARGET SYSTEM CALIB (dBZ0) LONG PULSE:   %6.2f dBZ\n", L2_adapt->tar_v_dbz0_inc_lp );
  printf( "  INITIAL DIFFERENTIAL PHASE:                   %5d   deg\n", L2_adapt->init_phi_dp );
  printf( "  NORMALIZED INITIAL DIFFERENTIAL PHASE:        %5d   deg\n", L2_adapt->norm_init_phi_dp );
  printf( "  MATCHED FILTER LOSS FOR LONG PULSE:           %5.2f dB\n", L2_adapt->lx_lp );
  printf( "  MATCHED FILTER LOSS FOR SHORT PULSE:          %5.2f dB\n", L2_adapt->lx_sp );
  printf( "  |K|**2 HYDROMETEOR REFRACTIVITY FACTOR:       %4.2f\n", L2_adapt->meteor_param );
  printf( "  ANTENNA BEAMWIDTH:                            %4.2f deg\n", L2_adapt->beamwidth );
  printf( "  ANTENNA GAIN (INCLUDING RADOME):              %5.2f dB\n", L2_adapt->antenna_gain );
  printf( "  VELOCITY CHECK DELTA DEGRADE LIMIT:           %3.1f m/s\n", L2_adapt->vel_degrad_limit );
  printf( "  SPECTRUM WIDTH CHECK DELTA DEGRADE LIMIT:     %3.1f m/s\n", L2_adapt->wth_degrad_limit );
  printf( "  HORIZ SYSTEM NOISE TEMP DEGRADE LIMIT:        %6.1f K\n", L2_adapt->h_noisetemp_dgrad_limit );
  printf( "  MINIMUM HORIZONTAL NOISE TEMPERATURE:         %5d   K\n", L2_adapt->h_min_noisetemp );
  printf( "  VERT SYSTEM NOISE TEMP DEGRADE LIMIT:         %6.1f K\n", L2_adapt->v_noisetemp_dgrad_limit );
  printf( "  MINIMUM VERTICAL NOISE TEMPERATURE:           %5d   K\n", L2_adapt->v_min_noisetemp );
  printf( "  KLYSTRON OUTPUT TARGET CONSIS DEGRADE LIMIT:  %4.1f dB\n", L2_adapt->kly_degrade_limit );
  printf( "  COHO POWER AT A1J4:                           %5.2f dBm\n", L2_adapt->ts_coho );
  printf( "  AME HORIZONTAL TEST SIGNAL POWER:             %5.2f dBm\n", L2_adapt->h_ts_cw );
  printf( "  STALO POWER AT A1J2:                          %5.2f dBm\n", L2_adapt->ts_stalo );
  printf( "  AME NOISE SOURCE HORIZ EXCESS NOISE RATIO:    %5.2f dB\n", L2_adapt->ame_h_noise_enr );
  printf( "  MAX TRANSMITTER PEAK POWER ALARM LEVEL:       %6.2f kW\n", L2_adapt->xmtr_peak_power_high_limit );
  printf( "  MIN TRANSMITTER PEAK POWER ALARM LEVEL:       %6.2f kW\n", L2_adapt->xmtr_peak_power_low_limit );
  printf( "  DIFF COMPUTED/TARGET HORIZ dBZ0 LIMIT:        %4.1f dB\n", L2_adapt->h_dbz0_delta_limit );
  printf( "  BYPASS MAP GENERATOR NOISE THRESHOLD:         %4.2f dB\n", L2_adapt->threshold1 );
  printf( "  BYPASS MAP GENERATOR REJECTION RATIO THRESH:  %4.2f dB\n", L2_adapt->threshold2 );
  printf( "  CLUTTER SUPPRESSION DEGRADED LIMIT:           %4.1f dB\n", L2_adapt->h_clut_supp_dgrad_lim );
  printf( "  TRUE RANGE AT FIRST RANGE BIN:                %5.3f km\n", L2_adapt->range0_value );
  printf( "  XMTR POWER BYTE DATA TO WATTS SCALE FACTOR:   %9.7f W\n", L2_adapt->xmtr_pwr_mtr_scale );
  printf( "  DIFF COMPUTED/TARGET VERT dBZ0 LIMIT:         %4.1f dB\n", L2_adapt->v_dbz0_delta_limit );
  printf( "  HORIZ TARGET SYSTEM CALIB (dBZ0) SHORT PULSE: %6.2f dB\n", L2_adapt->tar_h_dbz0_sp );
  printf( "  VERT TARGET SYSTEN CALIB (dBZ0) SHORT PULSE:  %6.2f dB\n", L2_adapt->tar_v_dbz0_sp );
  printf( "  SITE PRF SET:                                 %1d\n", L2_adapt->deltaprf );
  printf( "  PULSE WIDTH OF XMTR OUTPUT IN SHORT PULSE:    %4d nsec\n", L2_adapt->tau_sp );
  printf( "  PULSE WIDTH OF XMTR OUTPUT IN LONG PULSE:     %4d nsec\n", L2_adapt->tau_lp );
  printf( "  NUM 1/4km BINS OF CORRUPT DATA AT SWEEP END:  %2d bins\n", L2_adapt->nc_dead_value );
  printf( "  RF DRIVE PULSE WIDTH IN SHORT PULSE:          %4d nsec\n", L2_adapt->tau_rf_sp );
  printf( "  RF DRIVE PULSE WIDTH IN LONG PULSE:           %4d nsec\n", L2_adapt->tau_rf_lp );
  printf( "  CLUTTER MAP BOUNDARY ELEV BETWEEN SEG 1/2:    %4.2f deg\n", L2_adapt->seg1lim );
  printf( "  SITE LATITUDE:                                %2d:%02d:%7.4f\n", L2_adapt->slatdeg, L2_adapt->slatmin, L2_adapt->slatsec );
  printf( "  SITE LONGITUDE:                               %3d:%02d:%7.4f\n", L2_adapt->slongdeg, L2_adapt->slongmin, L2_adapt->slonsec );
  printf( "  SITE LATITUDE - DIRECTION:                    %s\n", L2_adapt->slatdir );
  printf( "  SITE LONGTUDE - DIRECTION:                    %s\n", L2_adapt->slondir );
  {
     double value = 0.0;
     memcpy( (char *) &value, (char *) &L2_adapt->dig_rcvr_clk_freq, sizeof(double) );
     printf( "  DIGITAL RECEIVER CLOCK FREQUENCY              %10.7lf MHz\n", value );

     memcpy( (char *) &value, (char *) &L2_adapt->coho_freq, sizeof(double) );
     printf( "  COHO FREQUENCY                                %10.7lf MHz\n", value );

  }
  printf( "  AZIMUTH BORESIGHT CORRECTION FACTOR:          %6.3f deg\n", L2_adapt->az_correction_factor );
  printf( "  ELEVATION BORESIGHT CORRECTION FACTOR:        %6.3f deg\n", L2_adapt->el_correction_factor );
  printf( "  ICAO:                                         %c%c%c%c\n", L2_adapt->site_name[0], L2_adapt->site_name[1], 
                                                           L2_adapt->site_name[2], L2_adapt->site_name[3] );
  printf( "  MINIMUM ELEVATION ANGLE:                      %9.5f deg\n", (float) L2_adapt->ant_manual_setup_ielmin*DEG_CONV );
  printf( "  MAXIMUM ELEVATION ANGLE:                      %9.5f deg\n", (float) L2_adapt->ant_manual_setup_ielmax*DEG_CONV );
  printf( "  MAXIMUM AZIMUTH VELOCITY:                     %3d deg/s\n", L2_adapt->ant_manual_setup_fazvelmax );
  printf( "  MAXIMUM ELEVATION VELOCITY:                   %3d deg/s\n", L2_adapt->ant_manual_setup_felvelmax );
  ...
  printf( "  AZIMUTH MOTOR POSITIVE SUSTAINING DRIVE:      %5.2f\n", L2_adapt->az_pos_sustain_drive );
  printf( "  AZIMUTH MOTOR NEGATIVE SUSTAINING DRIVE:      %5.2f\n", L2_adapt->az_neg_sustain_drive );
  printf( "  INITIAL EST FOR AZ POSITIVE DRIVE SLOPE:      %6.3f\n", L2_adapt->az_nom_pos_drive_slope );
  printf( "  INITIAL EST FOR AZ NEGATIVE DRIVE SLOPE:      %6.3f\n", L2_adapt->az_nom_neg_drive_slope );
  printf( "  AZIMUTH VELOCITY FEEDBACK SLOPE:              %6.3f\n", L2_adapt->az_feedback_slope );
  printf( "  ELEVATION MOTOR POSITIVE SUSTAINING DRIVE:    %5.2f\n", L2_adapt->el_pos_sustain_drive );
  printf( "  ELEVATION MOTOR NEGATIVE SUSTAINING DRIVE:    %5.2f\n", L2_adapt->el_neg_sustain_drive );
  printf( "  INITIAL EST FOR EL POSITIVE DRIVE SLOPE:      %6.3f\n", L2_adapt->el_nom_pos_drive_slope );
  printf( "  INITIAL EST FOR EL NEGATIVE DRIVE SLOPE:      %6.3f\n", L2_adapt->el_nom_neg_drive_slope );
  printf( "  ELEVATION VELOCITY FEEDBACK SLOPE:            %6.3f\n", L2_adapt->el_feedback_slope );
  printf( "  SLOPE FIRST INTERVAL EL POS FEEDBACK CURVE:   %5.2f\n", L2_adapt->el_first_slope );
  printf( "  SLOPE SECOND INTERVAL EL POS FEEDBACK CURVE:  %5.2f\n", L2_adapt->el_second_slope );
  printf( "  SLOPE THIRD INTERVAL EL POS FEEDBACK CURVE:   %5.2f\n", L2_adapt->el_third_slope );
  printf( "  NEUTRAL DROOP ANGLE:                          %5.2f\n", L2_adapt->el_droop_pos );
  printf( "  90 DEG OFF NEUTRAL DRIVE:                     %4.1f\n", L2_adapt->el_off_neutral_drive );
  printf( "  WAVEGUIDE LENGTH:                             %4d\n", L2_adapt->rvp8NV_iwaveguide_length );
  printf( "  AZIMUTH MOMENT OF INERTIA:                    %4.1f\n", L2_adapt->az_inertia );
  printf( "  ELEVATION MOMENT OF INERTIA:                  %4.1f\n", L2_adapt->el_inertia );
  printf( "  AZIMUTH STOW ANGLE FOR ENCODER ALIGNMENT      %4.1f deg\n", L2_adapt->az_stow_angle );
  printf( "  ELEVATION STOW ANGLE FOR ENCODER ALIGNMENT    %4.1f deg\n", L2_adapt->el_stow_angle );
  printf( "  AZIMUTH ENCODER ALIGNMENT ETU ANGLE AT STOW   %4.1f deg\n", L2_adapt->az_encoder_align );
  printf( "  ELEVATION ENCODER ALIGNMENT ETU ANGLE AT STOW %4.1f deg\n", L2_adapt->el_encoder_align );
  printf( "  REFINED PARK:                                 %s\n", L2_adapt->refined_park );

  for( i = 0; i <= 10; i++ )
  {
    printf( "  VERT RECEIVER NOISE NORMALIZATION-%2d:          %5.3f\n", i, L2_adapt->v_rnscale[i] );
  }
  ...
  for( i = 0; i <= 1; i++ )
  {
    printf( "  VERT RECEIVER NOISE NORMALIZATION-%2d:          %5.3f\n", 11+i, L2_adapt->v_rnscalec[i] );
  }
  printf( "  START RANGE FOR FIRST DOPPLER RADIAL:         %7.3f km\n", L2_adapt->doppler_range_start );
  printf( "  MAX INDEX FOR EL_INDEX PARAMETERS:            %2d\n", L2_adapt->max_el_index );
  printf( "  CLUTTER MAP BOUNDARY ELEV SEG 2/3:            %4.2f deg\n", L2_adapt->seg2lim );
  printf( "  CLUTTER MAP BOUNDARY ELEV SEG 3/4:            %4.2f deg\n", L2_adapt->seg3lim );
  printf( "  CLUTTER MAP BOUNDARY ELEV SEG 4/5:            %4.2f deg\n", L2_adapt->seg4lim );
  printf( "  NUM ELEVATION SEGMENTS IN CLUTTER MAP:        %1d\n", L2_adapt->nbr_el_segments );
  printf( "  HORIZONTAL RECEIVER NOISE FOR LONG PULSE:     %5.1f dBm\n", L2_adapt->h_noise_long );
  printf( "  ANTENNA NOISE TEMPERATURE:                    %5.1f K\n", L2_adapt->ant_noise_temp );
  printf( "  HORIZONTAL RECEIVER NOISE FOR SHORT PULSE:    %5.1f dBm\n", L2_adapt->h_noise_short );
  printf( "  HORIZONTAL RECEIVER NOISE TOLERANCE:          %3.1f dB\n", L2_adapt->h_noise_tolerance );
  printf( "  MINIMUM HORIZONTAL DYNAMIC RANGE:             %4.1f dB\n", L2_adapt->h_min_dyn_range );
  printf( "  AUXILLARY GENERATOR INSTALLED (FAA):          %s\n", L2_adapt->gen_installed );
  printf( "  AUXILLARY GENERATOR AUTO EXER ENABLED (FAA):  %s\n", L2_adapt->gen_exercies );
  printf( "  VERTICAL RECEIVER NOISE TOLERANCE:            %3.1f dB\n", L2_adapt->v_noise_tolerance );
  printf( "  MINIMUM VERTICAL DYNAMIC RANGE:               %4.1f dB\n", L2_adapt->v_min_dyn_range );
  printf( "  SYSTEM ZDR BIAS DEGRADE LIMIT:                %4.1f dB\n", L2_adapt->zdr_bias_degraded_lim );
  printf( "  BASELINE SYS DIFFERENTIAL REF BIAS:           %5.2f dB\n", L2_adapt->baseline_zdr_bias );
  printf( "  VERTICAL RECEIVER NOISE FOR LONG PULSE:       %5.1f dBm\n", L2_adapt->v_noise_long );
  printf( "  VERTICAL RECEIVER NOISE FOR SHORT PULSE:      %5.1f dBm\n", L2_adapt->v_noise_short );
  ...
  printf( "  STALO POWER DEGRADE LIMIT:                    %4.2f V\n", L2_adapt->stalo_pwr_dgrad_lim );
  printf( "  STALO POWER MAINT LIMIT:                      %4.2f V\n", L2_adapt->stalo_pwr_maint_lim );
  printf( "  MINIMUM HORIZONTAL POWER SENSE:               %5.2f dBm\n", L2_adapt->min_h_pwr_sense );
  printf( "  MINIMUM VERTICAL POWER SENSE:                 %5.2f dBm\n", L2_adapt->min_v_pwr_sense );
  printf( "  HORIZONTAL POWER SENSE CALIBRATION OFFSET:    %7.2f dB\n", L2_adapt->min_h_pwr_sense_off );
  printf( "  VERTICAL POWER SENSE CALIBRATION OFFSET:      %7.2f dB\n", L2_adapt->min_v_pwr_sense_off );
  printf( "  POWER SENSE GAIN REFERENCE VALUE:             %7.2f dB\n", L2_adapt->pwr_sense_gain_ref );
  ...
  printf( "  AME POWER SUPPLY TOLERANCE:                   %4.1f %%\n", L2_adapt->ame_ps_tolerance );
  printf( "  MAXIMUM AME INTERNAL ALARM TEMPERATURE:       %4.1f C\n", L2_adapt->ame_max_temp );
  printf( "  MINIMUM AME INTERNAL ALARM TEMPERATURE:       %4.1f C\n", L2_adapt->ame_min_temp );
  printf( "  MAXIMUM AME RECEIVER MODULE ALARM TEMP:       %4.1f C\n", L2_adapt->rcvr_mod_max_temp );
  printf( "  MINIMUM AME RECEIVER MODULE ALARM TEMP:       %4.1f C\n", L2_adapt->rcvr_mod_min_temp );
  printf( "  MAXIMUM AME BITE MODULE ALARM TEMP:           %4.1f C\n", L2_adapt->bite_mod_max_temp );
  printf( "  MINIMUM AME BITE MODULE ALARM TEMP:           %4.1f C\n", L2_adapt->bite_mod_min_temp );
  printf( "  DEFAULT (H+V) RF PALLET PHASE SHIFTER POS:    %d\n", L2_adapt->default_polarization );
  printf( "  TR LIMITER DEGRADE LIMIT:                     %5.2f V\n", L2_adapt->h_tr_limit_degraded_lim );
  printf( "  TR LIMITER FAILURE LIMIT:                     %5.2f V\n", L2_adapt->h_tr_limit_fail_lim );
  printf( "  AME PELTIER CURRENT TOLERANCE:                %5.1f %%\n", L2_adapt->ame_current_tol );
  printf( "  HORIZ (H ONLY) RF PALLET PHASE SHIFTER POS:   %d\n", L2_adapt->h_only_polarization );
  printf( "  VERT (V ONLY) RF PALLET PHASE SHIFTER POS:    %d\n", L2_adapt->v_only_polarization );
  printf( "  SUN MEASUREMENT BIAS:                         %6.2f dB\n", L2_adapt->reflector_bias );
  printf( "  LOW EQUIPMENT SHELTER TEMP WARNING LIMIT:     %6.2f dB\n", L2_adapt->a_min_shelter_temp_warn );
  printf( "  POWER METER 0 BIAS VOLTAGE:                   %6.2f dB\n", L2_adapt->pwr_meter_zero );
  printf( "  TRANSMIT BIAS BASELINE:                       %7.3f dB\n", L2_adapt->txb_baseline );
  printf( "  TRANSMIT BIAS ALARM THRESHOLD:                %7.3f dB\n", L2_adapt->txb_alarm_threshold );
  printf( "  NORMAL TPS POWER TIME:                        %d sec\n", L2_adapt->normal_tps_power_time );
  for( i = 0; i < 105; i++ )
  {
/*
...
/************************************************************************
 Description: Print RDA PRF Table message.
 ************************************************************************/

	//prfflag, This memcpy is the reason for the current segmenation fault.

int libL2_print_rda_prf_tables()
{
  int i, j = 0;
  float prf = 0.0f, prt = 0.0f, unam_rng_kilo = 0.0f, unam_rng_naut = 0.0f, nyq_velo = 0.0f;
  char waveform_title[] = "Contiguous Surveillance";

  printf( "RDA PRF TABLES MSG (MSG TYPE 32)\n" );
  printf( "Number Waveforms: %d\n", L2_prf_tables->n_waveforms );

  for (i = 0; i < L2_prf_tables->n_waveforms; i++)
  {
      if (L2_prf_tables->waveforms[i].waveform_code == 1)
      {
        strcpy(waveform_title, "Contiguous Surveillance");
      }
      else if (L2_prf_tables->waveforms[i].waveform_code == 2)
      {
        strcpy(waveform_title, "Contiguous Doppler");
      }
      else if (L2_prf_tables->waveforms[i].waveform_code == 5)
      {
        strcpy(waveform_title, "Staggered PRT");
      }

      printf("\n  PRF WAVEFORM CODE:       %s \n", waveform_title);
      printf("   PRF#     PPS       PRT(ms)       Rmax(km)     Rmax(nm)    Nyquist(m/s) \n");
      for (j = 0; j < L2_prf_tables->waveforms[i].n_prfs; j++)
      {
        /* prf is is millisecs. */
        prf = L2_prf_tables->waveforms[i].prfs[j];
        
        // print out PRF Data, comes out as digits so attached words to make sure that the info is actually readable.

        prt = (1.0f / prf) * pow(10, 6);
        unam_rng_kilo = (300.0f * prt) / 2.0f;
        unam_rng_naut = unam_rng_kilo / 1.852;
        
        /* Assumes 10 cm wavelength. */
        nyq_velo = prf / 40000.0;

        printf("   %2d:   %8.3f       %4.2f         %6.2f       %6.2f         %4.2f \n",
                j+1, prf/1000.0, prt, unam_rng_kilo, unam_rng_naut, nyq_velo);
      }
  }
...
int libL2_print_rda_clutter_map( int segment_number, int radial_number )
{
  ...
  static char *op_code[] = { "No Filter  ", 
                             "Bypass Map ", 
                             "All Bins   " };

  if( ( ret = Validate_clutter_map_segment_number( segment_number ) ) != LIBL2_NO_ERROR )
  {
    printf( "%s: Validate_clutter_map_segment_number returns %d\n", fx, ret );
    return ret;
  }
  else if( ( ret = Validate_clutter_map_radial_number( segment_number, radial_number ) ) != LIBL2_NO_ERROR )
  {
    printf( "%s: Validate_clutter_map_radial_number returns %d\n", fx, ret );
    return ret;
  }
    ...
    for( j = 0; j < libL2_clutter_map_num_azimuths(i+1); j++ )
    {
          ...
          ( radial_number == LIBL2_ALL_RADIALS || radial_number == j+1 ) )
      {
        for( k = 0; k < clutter_elev_segment->num_zones; k++ )
        {
          ...
          if( k == 0 )
             printf( "  Elev Segment: %1d,  Azimuth: %3d,  Zone: %2d,  OP Code: %s,  End Range: %3d km\n", 
                     i+1, j+1, k+1, op_code[clutter_filter->op_code], clutter_filter->range );
          else
             printf( "                                   Zone: %2d,  OP Code: %s,  End Range: %3d km\n", 
                     k+1, op_code[clutter_filter->op_code], clutter_filter->range );
        }
      }
...
/*
  ...
  printf( "  SPARE-8:              %d\n", elev_data.spare8 );
*/
  printf( "  VCP SEQUENCE:         %x\n", elev_data.spare9 );
  printf( "  VCP SUPPLEMENTAL:     %x\n", elev_data.spare10 );
/*
  printf( "  SPARE-11              %d\n", elev_data.spare11 );
*/
  printf( "\n" );
  ...
  for( i = 0; i < elev_data.number_cuts; i++ )
  {
    ...
    us = elev_data.data[i].edge_angle1;
...
*/
  ...
  printf( "  T1 INPUT FRAMES:                    %d octets\n", L2_pmd->t1_input_frames );
  printf( "  ROUTER MEMORY USED:                 %d bytes\n", L2_pmd->router_mem_used );
  printf( "  ROUTER MEMORY FREE:                 %d bytes\n", L2_pmd->router_mem_free );
  printf( "  ROUTER MEMORY UTILIZATION:          %d %%\n", L2_pmd->router_mem_util );
  s = L2_pmd->route_to_RPG;
  if( s == 0 )
  {
    printf( "  ROUTE TO RPG:                        NORMAL (%d)\n", s );
  }
  if( s == 1 )
  {
    printf( "  ROUTE TO RPG:                        BACKUP IN USE (%d)\n", s );
  }
  if( s == 2 )
  {
    printf( "  ROUTE TO RPG:                        BACKUP DOWN FAILURE (%d)\n", s );
  }
  if( s == 3 )
  {
    printf( "  ROUTE TO RPG:                        BACKUP COMMANDED DOWN (%d)\n", s );
  }
  if( s == 4 )
  {
    printf( "  ROUTE TO RPG:                        BACKUP NOT INSTALLED (%d)\n", s );
  }

  s = L2_pmd->router_status;
  if( s == 1 )
  {
    printf( "  T1 PORT STATUS:                      UP (%d)\n", s );
  }
  if( s == 2 )
  {
    printf( "  T1 PORT STATUS:                      DOWN (%d)\n", s );
  }
  if( s == 3 )
  {
    printf( "  T1 PORT STATUS:                      TEST (%d)\n", s );
  }

  s = L2_pmd->router_dedicated_ethernet_port_status;
  if( s == 1 )
  {
    printf( "  RTR DED ETHERNET PORT STATUS:        UP (%d)\n", s );
  }
  if( s == 2 )
  {
    printf( "  RTR DED ETHERNET PORT STATUS:        DOWN (%d)\n", s );
  }
  if( s == 3 )
  {
    printf( "  RTR DED ETHERNET PORT STATUS:        TEST (%d)\n", s );
  }

  s = L2_pmd->router_commercial_ethernet_port_status;
  if( s == 1 )
  {
    printf( "  RTR COM ETHERNET PORT STATUS:        UP (%d)\n", s );
  }
  if( s == 2 )
  {
    printf( "  RTR COM ETHERNET PORT STATUS:        DOWN (%d)\n", s );
  }
  if( s == 3 )
  {
    printf( "  RTR COM ETHERNET PORT STATUS:        TEST (%d)\n", s );
  }
//printf( "  SPARE16:                            %d\n", L2_pmd->spare16 );
//printf( "  SPARE17:                            %d\n", L2_pmd->spare17 );
//printf( "  SPARE18:                            %d\n", L2_pmd->spare18 );
...
//printf( "  SPARE20:                            %d\n", L2_pmd->spare20 );
  ...
  printf( "  CSU 24 HR LINE ERRORED SECONDS:     %d sec\n", L2_pmd->csu_24hr_line_err_scnds );
  printf( "  CSU 24 HR BURSTY ERRORED SECS:      %d sec\n", L2_pmd->csu_24hr_brsty_err_scnds );
  printf( "  CSU 24 HR DEGRADED MINUTES:         %d min\n", L2_pmd->csu_24hr_degraded_mins );
  printf( "  LAN SWITCH CPU UTILIZATION:         %d %%\n", L2_pmd->lan_switch_cpu_util );
  printf( "  LAN SWITCH MEM UTILIZATION:         %d %%\n", L2_pmd->lan_switch_mem_util );
//printf( "  SPARE-44:                           %d\n", L2_pmd->spare44 );
  printf( "  IFDR CHASSIS TEMPERATURE:           %d deg C\n", L2_pmd->ifdr_chassis_temp );
  printf( "  IFDR FPGA TEMPERATURE:              %d deg C\n", L2_pmd->ifdr_fpga_temp );
  printf( "  NTP STATUS:                         %d\n", L2_pmd->ntp_status );
/*
  printf( "  SPARE48:                            %d\n", L2_pmd->spare48 );
  printf( "  SPARE49:                            %d\n", L2_pmd->spare49 );
...
*/
  ...
  if( s == 0 )
  {
    printf( "  COMMANDED CHANNEL CONTROL:          N/A (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  COMMANDED CHANNEL CONTROL:          CH 1 (%d)\n", s );
  }
  else if( s == 2 )
  {
    printf( "  COMMANDED CHANNEL CONTROL:          CH 2 (%d)\n", s );
  }
  else
  {
    printf( "  COMMANDED CHANNEL CONTROL:          UNKNOWN (%d)\n", s );
  }
  printf( "AME\n" );
  ...
  printf( "  AME INTERNAL TEMPERATURE:           %5.1f C\n", L2_pmd->internal_temp );
  printf( "  AME RECV MOD TEMPERATURE:           %5.1f C\n", L2_pmd->rec_module_temp );
  printf( "  AME BITE/CAL MODULE TEMPERATURE:    %5.1f C\n", L2_pmd->bite_cal_module_temp );
  printf( "  AME PELTIER PULSE WID MODULATION:   %3d %%\n", L2_pmd->peltier_pulse_width_modulation );

  us = L2_pmd->peltier_status;
  if( us == 0 )
  {
    printf( "  AME PELTIER STATUS:                 OFF (%d)\n", us );
  }
  else if( us == 1 )
  {
    printf( "  AME PELTIER STATUS:                 ON (%d)\n", us );
  }
  else
  {
    printf( "  AME PELTIER STATUS:                 UNKNOWN (%d)\n", us );
  }

  us = L2_pmd->a2d_converter_status;
  if( us == 0 )
  {
    printf( "  AME A/D CONVERTER STATUS:           OK (%d)\n", us );
  }
  else if( us == 1 )
  {
    printf( "  AME A/D CONVERTER STATUS:           FAIL (%d)\n", us );
  }
  else
  {
    printf( "  AME A/D CONVERTER STATUS:           UNKNOWN (%d)\n", us );
  }
  ...
  printf( "  AME +3.3V PS VOLTAGE:               %4.2f V\n", L2_pmd->p_33vdc_ps );
  printf( "  AME +5V PS VOLTAGE:                 %4.2f V\n", L2_pmd->p_50vdc_ps );
  printf( "  AME +6.5V PS VOLTAGE:               %4.2f V\n", L2_pmd->p_65vdc_ps );
  printf( "  AME +15V PS VOLTAGE:                %5.2f V\n", L2_pmd->p_150vdc_ps );
  printf( "  AME +48V PS VOLTAGE:                %5.2f V\n", L2_pmd->p_480vdc_ps );
  printf( "  AME STALO POWER:                    %4.2f V\n", L2_pmd->stalo_power );
  printf( "  PELTIER CURRENT:                    %5.2f A\n", L2_pmd->peltier_current );
  printf( "  ADC CALIB REFERENCE VOLTAGE:        %5.3f V\n", L2_pmd->adc_calib_ref_voltage );
  us = L2_pmd->mode;
  if( us == 0 )
  {
  ...
  if( us == 0 )
  {
    printf( "  AME PELTIER MODE:                   COOL (%d)\n", us );
  }
  else if( us == 1 )
  {
    printf( "  AME PELTIER MODE:                   HEAT (%d)\n", us );
  }
  else
  {
    printf( "  AME PELTIER MODE:                   UNKNOWN (%d)\n", us );
  }
  printf( "  AME PELT INTERIOR FAN CURRENT:      %4.2f A\n", L2_pmd->peltier_inside_fan_current );
  printf( "  AME PELT EXTERIOR FAN CURRENT:      %4.2f A\n", L2_pmd->peltier_outside_fan_current );
  printf( "  HORIZONTAL TR LIMITER VOLTAGE:      %4.2f V\n", L2_pmd->h_tr_limiter_voltage );
  printf( "  VERT TR LIMITER VOLTAGE:            %4.2f V\n", L2_pmd->v_tr_limiter_voltage );
  printf( "  ADC CALIBRATION OFFSET VOLTAGE:     %7.2f mV\n", L2_pmd->adc_calib_offset_voltage );
  printf( "  ADC CALIBRATION GAIN CORRECTION:    %5.3f\n", L2_pmd->adc_calib_gain_correction );
  printf( "RCP/SPIP Power Button Status\n" );
  us = L2_pmd->rcp_status;
  if( us == 0 )
  {
    printf( "  RCP STATUS:                         OK (%d)\n", us );
  }
  else if( us == 1 )
  {
    printf( "  RCP STATUS:                         NOT OK (%d)\n", us );
  }
  printf( "  RCP STRING:                         %16s\n", L2_pmd->rcp_string );
  us = L2_pmd->spip_pwr_buttons;
  if( (us & 0x0001) == 0 )
  {
    printf( "  SPIP DAQ POWER BUTTON:              ON\n" );
  }
  else
  {
    printf( "  SPIP DAQ POWER BUTTON:              OFF\n" );
  }
  if( (us & 0x0002) == 0 )
  {
    printf( "  SPIP PED POWER BUTTON:              ON\n" );
  }
  else
  {
    printf( "  SPIP PED POWER BUTTON:              OFF\n" );
  }
  /* FAA or NWS Redundant. */
  if( Channel == 9 ) /* Only report if channel 1. */
  {
    if( (us & 0x0003) == 0 )
    {
      printf( "  SPIP DAQ POWER BUTTON (CHAN 2):     ON\n" );
    }
    else
    {
      printf( "  SPIP DAQ POWER BUTTON (CHAN 2):     OFF\n" );
    } 
    if( (us & 0x0004) == 0 )
    {
      printf( "  SPIP PED POWER BUTTON (CHAN 2):     ON\n" );
    }
    else
    {
      printf( "  SPIP PED POWER BUTTON (CHAN 2):     OFF\n" );
    }
  }
  printf( "POWER\n" );
  printf( "  MASTER POWER ADMINISTRATOR LOAD:    %5.2f A\n", L2_pmd->master_pwr_admin_load );
  printf( "  EXPANSION POWER ADMIN LOAD:         %5.2f A\n", L2_pmd->exp_pwr_admin_load );
  for( i = 0; i < 24; i++ )
  {
/*
...
*/
  ...
  {
    printf( "  TRANSMITTER AVAILABLE:              NO (%d)\n", s );
  }
  else
  {
    printf( "  TRANSMITTER AVAILABLE:              UNKNOWN (%d)\n", s );
  }
  ...
  if( s == 0 )
  {
    printf( "  WG/PFN TRANSFER INTERLOCK:          OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  WG/PFN TRANSFER INTERLOCK:          OPEN (%d)\n", s );
  }
  else
  {
    printf( "  WG/PFN TRANSFER INTERLOCK:          UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->mntnce_mode;
  if( s == 0 )
  {
    printf( "  MAINTENANCE MODE:                   NO (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  MAINTENANCE MODE:                   YES (%d)\n", s );
  }
  else
  {
    printf( "  MAINTENANCE MODE:                   UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->mntnce_reqd;
  if( s == 0 )
  {
    printf( "  MAINTENANCE REQUIRED:               NO (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  MAINTENANCE REQUIRED:               REQUIRED (%d)\n", s );
  }
  else
  {
  ...
  if( s == 0 )
  {
    printf( "  CHARGING SYSTEM FAIL:               OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  CHARGING SYSTEM FAIL:               FAIL (%d)\n", s );
  }
  else
  {
    printf( "  CHARGING SYSTEM FAIL:               UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->invrs_diode_crnt;
  if( s == 0 )
  {
    printf( "  INVERSE DIODE CURRENT:              OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  INVERSE DIODE CURRENT:              FAIL (%d)\n", s );
  }
  else
  {
    printf( "  INVERSE DIODE CURRENT:              UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->trggr_amp;
  if( s == 0 )
  {
    printf( "  TRIGGER AMPLIFIER:                  OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  TRIGGER AMPLIFIER:                  FAIL (%d)\n", s );
  }
  else
  {
    printf( "  TRIGGER AMPLIFIER:                  UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->circulator_temp;
  if( s == 0 )
  {
    printf( "  CIRCULATOR TEMPERATURE:             OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  CIRCULATOR TEMPERATURE:             FAIL (%d)\n", s );
  }
  else
  {
    printf( "  CIRCULATOR TEMPERATURE:             UNKNOWN (%d)\n", s );
  }
  ...
  if( s == 0 )
  {
    printf( "  CABINET AIR TEMPERATURE:            OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  CABINET AIR TEMPERATURE:            FAIL (%d)\n", s );
  }
  else
  {
    printf( "  CABINET AIR TEMPERATURE:            UNKNOWN (%d)\n", s );
  }
  ...
  if( s == 0 )
  {
    printf( "  KLYSTRON CURRENT:                   OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  KLYSTRON CURRENT:                   FAIL (%d)\n", s );
  }
  else
  {
  ...
  if( s == 0 )
  {
    printf( "  KLYSTRON AIR TEMPERATURE:           OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  KLYSTRON AIR TEMPERATURE:           FAIL (%d)\n", s );
  }
  else
  {
    printf( "  KLYSTRON AIR TEMPERATURE:           UNKNOWN (%d)\n", s );
  }
  ...
  if( s == 0 )
  {
    printf( "  MODULATOR SWITCH MAINTENANCE:       OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  MODULATOR SWITCH MAINTENANCE:       REQUIRED (%d)\n", s );
  }
  else
  {
    printf( "  MODULATOR SWITCH MAINTENANCE:       UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->post_chrg_regulator;
  if( s == 0 )
  {
    printf( "  POST CHARGE REGULATOR MAINT:        OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  POST CHARGE REGULATOR MAINT:        MAINTENANCE (%d)\n", s );
  }
  else
  {
    printf( "  POST CHARGE REGULATOR MAINT:        UNKNOWN (%d)\n", s );
  }
  ...
  if( s == 0 )
  {
    printf( "  TRANSMITTER OVERVOLTAGE:            OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  TRANSMITTER OVERVOLTAGE:            OVER (%d)\n", s );
  }
  else
  {
    printf( "  TRANSMITTER OVERVOLTAGE:            UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->trsmttr_ovr_crnt;
  if( s == 0 )
  {
    printf( "  TRANSMITTER OVERCURRENT:            OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  TRANSMITTER OVERCURRENT:            OVER (%d)\n", s );
  }
  else
  {
    printf( "  TRANSMITTER OVERCURRENT:            UNKNOWN (%d)\n", s );
  } 
  ...
  if( s == 0 )
  {
    printf( "  OIL TEMPERATURE:                    OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  OIL TEMPERATURE:                    FAIL (%d)\n", s );
  }
  else
  {
    printf( "  OIL TEMPERATURE:                    UNKNOWN (%d)\n", s );
  } 
  s = L2_pmd->prf_limit;
  if( s == 0 )
  {
  ...
  if( s == 0 )
  {
    printf( "  TRANSMITTER OIL LEVEL:              OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  TRANSMITTER OIL LEVEL:              FAIL (%d)\n", s );
  }
  else
  {
    printf( "  TRANSMITTER OIL LEVEL:              UNKNOWN (%d)\n", s );
  } 
  s = L2_pmd->trsmttr_batt_chrgng;
  if( s == 0 )
  {
    printf( "  TRANSMITTER BATTERY CHARGING:       YES (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  TRANSMITTER BATTERY CHARGING:       NO (%d)\n", s );
  }
  else
  {
    printf( "  TRANSMITTER BATTERY CHARGING:       UNKNOWN (%d)\n", s );
  } 
  s = L2_pmd->hv_status;
  if( s == 0 )
  {
  ...
  if( s == 0 )
  {
    printf( "  TRANSMITTER RECYCLING SUMMARY:      NORMAL (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  TRANSMITTER RECYCLING SUMMARY:      RECYCLING (%d)\n", s );
  }
  else
  {
    printf( "  TRANSMITTER RECYCLING SUMMARY:      UNKNOWN (%d)\n", s );
  } 
  s = L2_pmd->trsmttr_inoperable;
  if( s == 0 )
  {
    printf( "  TRANSMITTER INOP:                   OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  TRANSMITTER INOP:                   INOP (%d)\n", s );
  }
  else
  {
    printf( "  TRANSMITTER INOP:                   UNKNOWN (%d)\n", s );
  } 
  s = L2_pmd->trsmttr_air_fltr;
  if( s == 0 )
  {
    printf( "  TRANSMITTER AIR FILTER:             DIRTY (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  TRANSMITTER AIR FILTER:             OK (%d)\n", s );
  }
  else
  {
    printf( "  TRANSMITTER AIR FILTER:             UNKNOWN (%d)\n", s );
  } 
  ...
  s = L2_pmd->xmtr_spip_interface;
  if( s == 0 )
  {
    printf( "  TRANSMITTER/SPIP INTERFACE:         FAIL (%d)\n", s);
  }
  else if( s == 1 )
  {
    printf( "  TRANSMITTER/SPIP INTERFACE:         OK (%d)\n", s);
  }
  else
  {
    printf( "  TRANSMITTER/SPIP INTERFACE:         UNKNOWN (%d)\n", s);
  }
  s = L2_pmd->trsmttr_smmry_status;
  if( s == 0 )
  {
    printf( "  TRANSMITTER SUMMARY STATUS:         READY (%d)\n", s);
  }
  else if( s == 1 )
  {
    printf( "  TRANSMITTER SUMMARY STATUS:         ALARM (%d)\n", s);
  }
  else if( s == 2 )
  {
    printf( "  TRANSMITTER SUMMARY STATUS:         MAINTENANCE (%d)\n", s);
  }
  else if( s == 3 )
  {
    printf( "  TRANSMITTER SUMMARY STATUS:         RECYCLE (%d)\n", s);
  }
  else if( s == 4 )
  {
    printf( "  TRANSMITTER SUMMARY STATUS:         PREHEAT (%d)\n", s);
  }
  else
  {
    printf( "  TRANSMITTER SUMMARY STATUS:         UNKNOWN (%d)\n", s);
  }
/*
  printf( "  SPARE-204:                          %d\n", L2_pmd->spare204 );
*/
  printf( "  TRANSMITTER RF POWER:               %7.4f mW\n", L2_pmd->trsmttr_rf_pwr );
  printf( "  HORIZONTAL TRANSMITTER PEAK POWER:  %5.1f kW\n", L2_pmd->h_trsmttr_peak_pwr );
  printf( "  TRANSMITTER PEAK POWER:             %5.1f kW\n", L2_pmd->xmtr_peak_pwr );
  printf( "  VERTICAL TRANSMITTER PEAK POWER:    %5.1f kW\n", L2_pmd->v_trsmttr_peak_pwr );
  printf( "  TRANSMITTER RF AVG POWER:           %6.1f W\n", L2_pmd->xmtr_rf_avg_pwr );
/*
  printf( "  SPARE-216:                          %d\n", L2_pmd->spare216 );
*/
  printf( "  TRANSMITTER RECYCLE COUNT:          %6d\n", L2_pmd->xmtr_recycle_cnt );
  printf( "  RECEIVER BIAS:                      %f\n", L2_pmd->receiver_bias );
  printf( "  TRANSMIT IMBALANCE:                 %f\n", L2_pmd->transmit_imbalance );
  printf( "  TRANSMITTER POWER METER ZERO:       %6.1f\n", L2_pmd->xmtr_pwr_mtr_zero );
  for( i = 0; i < 4; i++ )
  {
/*
...
*/
  ...
  if( s == 0 )
  {
    printf( "  AC UNIT #1 COMPRESSOR SHUTOFF:      OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AC UNIT #1 COMPRESSOR SHUTOFF:      SHUTOFF (%d)\n", s );
  }
  else
  {
    printf( "  AC UNIT #1 COMPRESSOR SHUTOFF:      UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->ac_2_cmprsr_shut_off;
  if( s == 0 )
  {
    printf( "  AC UNIT #2 COMPRESSOR SHUTOFF:      OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AC UNIT #2 COMPRESSOR SHUTOFF:      SHUTOFF (%d)\n", s );
  }
  else
  {
    printf( "  AC UNIT #2 COMPRESSOR SHUTOFF:      UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->gnrtr_mntnce_reqd;
  if( s == 0 )
  {
    printf( "  GENERATOR MAINTENANCE REQUIRED:     YES (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  GENERATOR MAINTENANCE REQUIRED:     NO (%d)\n", s );
  }
  else
  {
    printf( "  GENERATOR MAINTENANCE REQUIRED:     UNKNOWN (%d)\n", s );
  }
  ...
  if( s == 0 )
  {
    printf( "  TRANSITIONAL POWER SOURCE (TPS):    OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  TRANSITIONAL POWER SOURCE (TPS):    OFF (%d)\n", s );
  }
  else
  {
    printf( "  TRANSITIONAL POWER SOURCE (TPS):    UNKNOWN (%d)\n", s );
  }
  ...
  if( s == 0 )
  {
    printf( "  AIRCRAFT HAZARD LIGHTING:           FAIL (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AIRCRAFT HAZARD LIGHTING:           OK (%d)\n", s );
  }
  else
  {
    printf( "  AIRCRAFT HAZARD LIGHTING:           UNKNOWN (%d)\n", s );
  }
  for( i = 0; i < 11; i++ )
  {
/*
...
*/
  ...
  if( s == 0 )
  {
    printf( "  EQUIPMENT FIRE DETECT SYSTEM:       OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  EQUIPMENT FIRE DETECT SYSTEM:       FAIL (%d)\n", s );
  }
  else
  {
    printf( "  EQUIPMENT FIRE DETECT SYSTEM:       UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->equip_shltr_fire_smk;
  if( s == 0 )
  {
    printf( "  EQUIPMENT FIRE/SMOKE:               OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  EQUIPMENT FIRE/SMOKE:               FIRE (%d)\n", s );
  }
  else
  {
    printf( "  EQUIPMENT FIRE/SMOKE:               UNKNOWN (%d)\n", s );
  }
  ...
  if( s == 0 )
  {
    printf( "  UTILITY VOLTAGE/FREQUENCY:          NOT AVAILABLE (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  UTILITY VOLTAGE/FREQUENCY:          AVAILABLE (%d)\n", s );
  }
  else
  {
    printf( "  UTILITY VOLTAGE/FREQUENCY:          UNKNOWN (%d)\n", s );
  }
  ...
  if( s == 0 )
  {
    printf( "  SECURITY EQUIPMENT:                 FAIL (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  SECURITY EQUIPMENT:                 OK (%d)\n", s );
  }
  else
  {
    printf( "  SECURITY EQUIPMENT:                 UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->scrty_sys;
  if( s == 0 )
  {
    printf( "  SECURITY SYSTEM:                    DISABLED (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  SECURITY SYSTEM:                    OK (%d)\n", s );
  }
  else
  {
    printf( "  SECURITY SYSTEM:                    UNKNOWN (%d)\n", s );
  }
  ...
  {
    printf( "  RECEIVER CONNECTED TO ANTENNA:      NOT CONNECTED (%d)\n", s );
  }
  else if( s == 2 )
  {
    printf( "  RECEIVER CONNECTED TO ANTENNA:       N/A (%d)\n", s );
  }
  else
  {
    printf( "  RECEIVER CONNECTED TO ANTENNA:       UNKNOWN (%d)\n", s );
  }
  ...
  if( s == 0 )
  {
    printf( "  AC UNIT #1 FILTER DIRTY:            DIRTY (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AC UNIT #1 FILTER DIRTY:            OK (%d)\n", s );
  }
  else
  {
    printf( "  AC UNIT #1 FILTER DIRTY:            UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->ac_2_fltr_drty;
  if( s == 0 )
  {
    printf( "  AC UNIT #2 FILTER DIRTY:            DIRTY (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AC UNIT #2 FILTER DIRTY:            OK (%d)\n", s );
  }
  else
  {
    printf( "  AC UNIT #2 FILTER DIRTY:            UNKNOWN (%d)\n", s );
  }
  printf( "  EQUIPMENT SHELTER TEMPERATURE:      %5.2f C\n", L2_pmd->equip_shltr_temp );
  printf( "  OUTSIDE AMBIENT TEMPERATURE:        %6.2f C\n", L2_pmd->outside_amb_temp );
  printf( "  XMTR LEAVING AIR TEMPERATURE:       %6.2f C\n", L2_pmd->trsmttr_leaving_air_temp );
  printf( "  AC #1 DISCHARGE TEMPERATURE:        %5.2f C\n", L2_pmd->ac_1_dschrg_air_temp );
  printf( "  GENERATOR SHELTER TEMPERATURE:      %5.2f C\n", L2_pmd->gnrtr_shltr_temp );
  printf( "  RADOME AIR TEMPERATURE:             %6.2f C\n", L2_pmd->radome_air_temp );
  printf( "  AC #2 DISCHARGE TEMPERATURE:        %5.2f C\n", L2_pmd->ac_2_dschrg_air_temp );
  printf( "  SPIP +15v PS:                       %5.2f V\n", L2_pmd->spip_p_15v_ps );
  printf( "  SPIP -15v PS:                       %6.2f V\n", L2_pmd->spip_n_15v_ps );
  s = L2_pmd->spip_p_28v_ps;
  if( s == 0 )
  {
    printf( "  SPIP +28v PS STATUS:                OK (%d) \n",s );
  }
  else if( s == 1 )
  {
    printf( "  SPIP +28v PS STATUS:                FAIL (%d) \n", s );
  }
  else
  {
    printf( "  SPIP +28v PS STATUS:                UNKNOWN (%d) \n", s );
  }
  printf( "  SPIP  +5v PS:                       %4.2f V\n", L2_pmd->spip_p_5v_ps );
  printf( "  CONVERTED GENERATOR FUEL LEVEL:     %3d %%\n", L2_pmd->cnvrtd_gnrtr_fuel_lvl );
  for( i = 0; i < 7; i++ )
  {
/*
...
*/
  ...
  printf( "PEDESTAL\n" );
  s = L2_pmd->elev_p_dead_lmt;
  if( s == 0 )
  {
    printf( "  ELEVATION + DEAD LIMIT:             OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  ELEVATION + DEAD LIMIT:             IN LIMIT (%d)\n", s );
  }
  else 
  {
    printf( "  ELEVATION + DEAD LIMIT:             UNKNOWN (%d)\n", s );
  }
  ...
  if( s == 0 )
  {
    printf( "  ELEV SERVO AMP SHORT CIRUIT:        NORMAL (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  ELEV SERVO AMP SHORT CIRUIT:        SHORT CIRCUIT (%d)\n", s );
  }
  else
  {
    printf( "  ELEV SERVO AMP SHORT CIRUIT:        UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->elev_srvo_amp_ovr_temp;
  if( s == 0 )
  {
    printf( "  ELEVATION SERVO AMP OVERTEMP:       NORMAL (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  ELEVATION SERVO AMP OVERTEMP:       OVERTEMP (%d)\n", s );
  }
  else
  {
    printf( "  ELEVATION SERVO AMP OVERTEMP:       UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->elev_motor_ovr_temp;
  if( s == 0 )
  {
    printf( "  ELEVATION MOTOR OVERTEMP:           OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  ELEVATION MOTOR OVERTEMP:           OVERTEMP (%d)\n", s );
  }
  else
  {
    printf( "  ELEVATION MOTOR OVERTEMP:           NORMAL (%d)\n", s );
  }
  s = L2_pmd->elev_stow_pin;
  if( s == 0 )
  {
    printf( "  ELEVATION STOW PIN:                 OPERATIONAL (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  ELEVATION STOW PIN:                 ENGAGED (%d)\n", s );
  }
  else
  {
    printf( "  ELEVATION STOW PIN:                 UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->elev_housing_5v_ps;
  if( s == 0 )
  {
    printf( "  ELEVATION HOUSING 5V PS:            OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  ELEVATION HOUSING 5V PS:            FAIL (%d)\n", s );
  }
  else
  {
    printf( "  ELEVATION HOUSING 5V PS:            UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->elev_n_dead_lmt;
  if( s == 0 )
  {
    printf( "  ELEVATION -DEAD LIMIT:              OK (%d)\n", s );
  }
  ...
  if( s == 0 )
  {
    printf( "  ELEVATION +NORMAL LIMIT:            OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  ELEVATION +NORMAL LIMIT:            IN LIMIT (%d)\n", s );
  }
  else
  {
    printf( "  ELEVATION +NORMAL LIMIT:            UNKNOWN (%d)\n", s );
  }
  ...
  if( s == 0 )
  {
    printf( "  ELEVATION ENCODER LIGHT:            FAIL (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  ELEVATION ENCODER LIGHT:            OK (%d)\n", s );
  }
  else
  {
    printf( "  ELEVATION ENCODER LIGHT:            UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->elev_grbx_oil;
  if( s == 0 )
  {
    printf( "  ELEVATION GEARBOX OIL:              OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  ELEVATION GEARBOX OIL:              OIL LEVEL LOW (%d)\n", s );
  }
  else
  {
    printf( "  ELEVATION GEARBOX OIL:              UNKNOWN (%d)\n", s );
  }

  s = L2_pmd->elev_handwheel;
  if( s == 0 )
  {
    printf( "  ELEVATION HANDWHEEL:                OPERATIONAL (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  ELEVATION HANDWHEEL:                ENGAGED (%d)\n", s );
  }
  else
  {
    printf( "  ELEVATION HANDWHEEL:                UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->elev_amp_ps;
  if( s == 0 )
  {
    printf( "  ELEVATION AMP PS:                   OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  ELEVATION AMP PS:                   FAIL (%d)\n", s );
  }
  else
  {  
    printf( "  ELEVATION AMP PS:                   UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->azmth_srvo_amp_inhbt;
  if( s == 0 )
  {
    printf( "  AZIMUTH SERVO AMP INHIBIT:          OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AZIMUTH SERVO AMP INHIBIT:          INHIBIT (%d)\n", s );
  }
  else
  {  
    printf( "  AZIMUTH SERVO AMP INHIBIT:          UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->azmth_srvo_amp_shrt_crct;
  if( s == 0 )
  {
    printf( "  AZIMUTH SERVO AMP SHORT CIR:        SHORT CIRCUIT (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AZIMUTH SERVO AMP SHORT CIR:        OK (%d)\n", s );
  }
  else
  {  
    printf( "  AZIMUTH SERVO AMP SHORT CIR:        UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->azmth_srvo_amp_ovr_temp;
  if( s == 0 )
  {
    printf( "  AZIMUTH SERVO OVERTEMP:             OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AZIMUTH SERVO OVERTEMP:             OVERTEMP (%d)\n", s );
  }
  else
  {
    printf( "  AZIMUTH SERVO OVERTEMP:             UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->azmth_motor_ovr_temp;
  if( s == 0 )
  {
    printf( "  AZIMUTH MOTOR OVERTEMP:             OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AZIMUTH MOTOR OVERTEMP:             OVERTEMP (%d)\n", s );
  }
  else
  {
    printf( "  AZIMUTH MOTOR OVERTEMP:             UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->azmth_stow_pin;
  if( s == 0 )
  {
    printf( "  AZIMUTH STOW PIN:                   OPERATIONAL (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AZIMUTH STOW PIN:                   ENGAGED (%d)\n", s );
  }
  else
  {
    printf( "  AZIMUTH STOW PIN:                   UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->azmth_housing_5v_ps;
  if( s == 0 )
  {
    printf( "  AZIMUTH HOUSING 5V PS:              OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AZIMUTH HOUSING 5V PS:              FAIL (%d)\n", s );
  }
  else
  {
    printf( "  AZIMUTH HOUSING 5V PS:              UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->azmth_encdr_light;
  if( s == 0 )
  {
    printf( "  AZIMUTH ENCODER LIGHT:              OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AZIMUTH ENCODER LIGHT:              FAIL (%d)\n", s );
  }
  else
  {
    printf( "  AZIMUTH ENCODER LIGHT:              UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->azmth_grbx_oil;
  if( s == 0 )
  {
    printf( "  AZIMUTH GEARBOX OIL:                OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AZIMUTH GEARBOX OIL:                OIL LEVEL LOW (%d)\n", s );
  }
  else
  {
    printf( "  AZIMUTH GEARBOX OIL:                UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->azmth_bull_gr_oil;
  if( s == 0 )
  {
    printf( "  AZIMUTH BULL GEAR OIL:              OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AZIMUTH BULL GEAR OIL:              OIL LEVEL LOW (%d)\n", s );
  }
  else
  {
    printf( "  AZIMUTH BULL GEAR OIL:              UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->azmth_handwheel;
  if( s == 0 )
  {
    printf( "  AZIMUTH HANDWHEEL:                  OPERATIONAL (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AZIMUTH HANDWHEEL:                  ENGAGED (%d)\n", s );
  }
  else
  {
    printf( "  AZIMUTH HANDWHEEL:                  UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->azmth_srvo_amp_ps;
  if( s == 0 )
  {
    printf( "  AZIMUTH SERVO AMP PS:               OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AZIMUTH SERVO AMP PS:               FAIL (%d)\n", s );
  }
  else
  {
    printf( "  AZIMUTH SERVO AMP PS:               UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->srvo;
  if( s == 0 )
  {
  ...
  if( s == 0 )
  {
    printf( "  PEDESTAL INTERLOCK SWITCH:          OPERATIONAL (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  PEDESTAL INTERLOCK SWITCH:          SAFE (%d)\n", s );
  }
  else
  {
    printf( "  PEDESTAL INTERLOCK SWITCH:          UNKNOWN (%d)\n", s );
  }
  for( i = 0; i < 12; i++ )
  {
/*
...
*/
  ...
  if( s == 0 )
  {
    printf( "  RF GEN FREQ SELECT OSCILLATOR:      OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  RF GEN FREQ SELECT OSCILLATOR:      FAIL (%d)\n", s );
  }
  else
  {
    printf( "  RF GEN FREQ SELECT OSCILLATOR:      UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->rf_gnrtr_rf_stalo;
  if( s == 0 )
  {
    printf( "  RF GENERATOR RF/STALO:              OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  RF GENERATOR RF/STALO:              FAIL (%d)\n", s );
  }
  else
  {
    printf( "  RF GENERATOR RF/STALO:              UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->rf_gnrtr_phase_shft_coho;
  if( s == 0 )
  {
    printf( "  RF GENERATOR PHASE SHIFT COHO:      OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  RF GENERATOR PHASE SHIFT COHO:      FAIL (%d)\n", s );
  }
  else
  {
    printf( "  RF GENERATOR PHASE SHIFT COHO:      UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->p_9v_rcvr_ps;
  if( s == 0 )
  {
    printf( "  +9v RECEIVER PS:                    OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  +9v RECEIVER PS:                    FAIL (%d)\n", s );
  }
  else
  {
    printf( "  +9v RECEIVER PS:                    UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->p_5v_rcvr_ps;
  if( s == 0 )
  {
    printf( "  +5v RECEIVER PS:                    OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  +5v RECEIVER PS:                    FAIL (%d)\n", s );
  }
  else
  {
    printf( "  +5v RECEIVER PS:                    UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->pn_18v_rcvr_ps;
  if( s == 0 )
  {
    printf( "  +/-18v RECEIVER PS:                 OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  +/-18v RECEIVER PS:                 FAIL (%d)\n", s );
  }
  else
  {
    printf( "  +/-18v RECEIVER PS:                 UNKNOWN (%d)\n", s );
  }
  ...
  if( s == 0 )
  {
    printf( "  +5v RECEIVER PROTECT PS:            OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  +5v RECEIVER PROTECT PS:            FAIL (%d)\n", s );
  }
  else
  {
    printf( "  +5v RECEIVER PROTECT PS:            UNKNOWN (%d)\n", s );
  }
/*
  printf( "  SPARE-350:                          %d\n", L2_pmd->spare350 );
*/
  printf( "  HORIZONTAL SHORT PULSE NOISE:       %7.2f dBm\n", L2_pmd->h_shrt_pulse_noise );
  printf( "  HORIZONTAL LONG PULSE NOISE:        %7.2f dBm\n", L2_pmd->h_long_pulse_noise );
  printf( "  HORIZONTAL NOISE TEMPERATURE:       %7.2f K\n", L2_pmd->h_noise_temp );
  printf( "  VERTICAL SHORT PULSE NOISE:         %7.2f dBm\n", L2_pmd->v_shrt_pulse_noise );
  printf( "  VERTICAL LONG PULSE NOISE:          %7.2f dBm\n", L2_pmd->v_long_pulse_noise );
  printf( "  VERTICAL NOISE TEMPERATURE:         %7.2f K\n", L2_pmd->v_noise_temp );
  printf( "CALIBRATION\n" );
  printf( "  HORIZONTAL LINEARITY:               %6.4f\n", L2_pmd->h_linearity );
  printf( "  HORIZONTAL DYNAMIC RANGE:           %7.3f dB\n", L2_pmd->h_dynamic_range );
  printf( "  HORIZONTAL DELTA dBZ0:              %7.2f dB\n", L2_pmd->h_delta_dbz0 );
  printf( "  VERTICAL DELTA dBZ0:                %7.2f dB\n", L2_pmd->v_delta_dbz0 );
...
*/
  ...
  printf( "  HORIZONTAL SHORT PULSE dBZ0:        %8.4f dBZ\n", L2_pmd->shrt_pls_h_dbz0 );
  printf( "  HORIZONTAL LONG PULSE dBZ0:         %8.4f dBZ\n", L2_pmd->long_pls_h_dbz0 );
  s = L2_pmd->velocity_prcssd;
  if( s == 0 )
  {
    printf( "  VELOCITY (PROCESSED):               GOOD (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  VELOCITY (PROCESSED):               FAIL (%d)\n", s );
  }
  else
  {
    printf( "  VELOCITY (PROCESSED):               UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->width_prcssd;
  if( s == 0 )
  {
    printf( "  WIDTH (PROCESSED):                  GOOD (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  WIDTH (PROCESSED):                  FAIL (%d)\n", s );
  }
  else
  {
    printf( "  WIDTH (PROCESSED):                  UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->velocity_rf_gen;
  if( s == 0 )
  {
    printf( "  VELOCITY (RF GEN):                  GOOD (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  VELOCITY (RF GEN):                  FAIL (%d)\n", s );
  }
  else
  {
    printf( "  VELOCITY (RF GEN):                  UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->width_rf_gen;
  if( s == 0 )
  {
    printf( "  WIDTH (RF GEN):                     GOOD (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  WIDTH (RF GEN):                     FAIL (%d)\n", s );
  }
  else
  {
    printf( "  WIDTH (RF GEN):                     UNKNOWN (%d)\n", s );
  }
  printf( "  HORIZONTAL I0:                      %9.4f dBm\n", L2_pmd->h_i_naught );
  printf( "  VERTICAL I0:                        %9.4f dBm\n", L2_pmd->v_i_naught );
  printf( "  VERTICAL DYNAMIC RANGE:             %7.3f dB\n", L2_pmd->v_dynamic_range );
  printf( "  VERTICAL SHORT PULSE dBZ0:          %8.4f dBZ\n", L2_pmd->shrt_pls_v_dbz0 );
  printf( "  VERTICAL LONG PULSE dBZ0:           %8.4f dBZ\n", L2_pmd->long_pls_v_dbz0 );
  for( i = 0; i < 4; i++ )
  {
/*
...
*/
  ...
  printf( "  HORIZONTAL POWER SENSE:             %9.4f dBm\n", L2_pmd->h_pwr_sense );
  printf( "  VERTICAL POWER SENSE:               %9.4f dBm\n", L2_pmd->v_pwr_sense );
  printf( "  ZDR OFFSET:                         %9.4f dB\n", L2_pmd->zdr_bias );
  for( i = 0; i < 6; i++ )
  {
/*
...
*/
  ...
  printf( "  HORIZONTAL CLUTT SUPP DELTA:        %6.2f dB\n", L2_pmd->h_cltr_supp_delta );
  printf( "  HORIZONTAL CLUTT SUPP UNFILT PWR:   %6.2f dBZ\n", L2_pmd->h_cltr_supp_ufilt_pwr );
  printf( "  HORIZONTAL CLUTT SUPP FILT PWR:     %6.2f dBZ\n", L2_pmd->h_cltr_supp_filt_pwr );
/*
  printf( "  SPARE-415[0]:                       %d\n", L2_pmd->spare415[0] );
  printf( "  SPARE-415[1]:                       %d\n", L2_pmd->spare415[1] );
  printf( "  SPARE-419[0]:                       %d\n", L2_pmd->spare419[0] );
  printf( "  SPARE-419[1]:                       %d\n", L2_pmd->spare419[1] );
  printf( "  SPARE-419[2]:                       %d\n", L2_pmd->spare419[2] );
  printf( "  SPARE-419[3]:                       %d\n", L2_pmd->spare419[3] );
  printf( "  SPARE-419[4]:                       %d\n", L2_pmd->spare419[4] );
  printf( "  SPARE-419[5]:                       %d\n", L2_pmd->spare419[5] );
*/
  printf( "  VERTICAL LINEARITY:                 %6.4f\n", L2_pmd->v_linearity );
  for( i = 0; i < 4; i++ )
  {
/*
...
*/
  ...
  if( s == 0 )
  {
    printf( "  BYPASS MAP READ STATUS:             OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  BYPASS MAP READ STATUS:             FAIL (%d)\n", s );
  }
  else
  {
    printf( "  BYPASS MAP READ STATUS:             UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->bypass_map_file_wrt_stat;
  if( s == 0 )
  {
    printf( "  BYPASS MAP WRITE STATUS:            OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  BYPASS MAP WRITE STATUS:            FAIL (%d)\n", s );
  }
  else
  {
    printf( "  BYPASS MAP WRITE STATUS:            UNKNOWN (%d)\n", s );
  }
/*
  printf( "  SPARE-435:                          %d\n", L2_pmd->spare435 );
  printf( "  SPARE-436:                          %d\n", L2_pmd->spare436 );
*/
  ...
  if( s == 0 )
  {
    printf( "  CURRENT ADAPT FILE READ STATUS:     OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  CURRENT ADAPT FILE READ STATUS:     FAIL (%d)\n", s );
  }
  else
  {
    printf( "  CURRENT ADAPT FILE READ STATUS:     UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->crnt_adpt_file_wrt_stat;
  if( s == 0 )
  {
    printf( "  CURRENT ADAPT FILE WRITE STATUS:    OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  CURRENT ADAPT FILE WRITE STATUS:    FAIL (%d)\n", s );
  }
  else
  {
    printf( "  CURRENT ADAPT FILE WRITE STATUS:    UNKNOWN (%d)\n", s );
  }
  ...
  s = L2_pmd->prf_set_rd_stat;
  if( s & 0x1 )
  {
    printf( "  READ STATUS OF SURV PRF SET:        OK (%d)\n", (s & 0x1) );
  }
  else 
  {
    printf( "  READ STATUS OF SURV PRF SET:        FAIL (%d)\n", (s & 0x1) );
  }
  if( s & 0x2 )
  {
    printf( "  READ STATUS OF DOPPLER PRF SET:     OK (%d)\n", (s & 0x2) );
  }
  else 
  {
    printf( "  READ STATUS OF DOPPLER PRF SET:     FAIL (%d)\n", (s & 0x2) );
  }
  if( s & 0x4 )
  {
    printf( "  READ STATUS OF STP PRF SET:         OK (%d)\n", (s & 0x4) );
  }
  else 
  {
    printf( "  READ STATUS OF STP PRF SET:         FAIL (%d)\n", (s & 0x4) );
  }
  s = L2_pmd->cf_map_file_rd_stat;
  if( s == 0 )
  {
    printf( "  CLUTTER FILT MAP FILE READ STATUS:  OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  CLUTTER FILT MAP FILE READ STATUS:  FAIL (%d)\n", s );
  }
  else
  {
    printf( "  CLUTTER FILT MAP FILE READ STATUS:  UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->cf_map_file_wrt_stat;
  if( s == 0 )
  {
    printf( "  CLUT FILT MAP FILE WRITE STATUS:    OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  CLUT FILT MAP FILE WRITE STATUS:    FAIL (%d)\n", s );
  }
  else
  {
    printf( "  CLUT FILT MAP FILE WRITE STATUS:    UNKNOWN (%d)\n", s );
  }
...
*/
  ...
  s = L2_pmd->spip_comm_stat;
  if( s == 0 )
  {
    printf( "  SPIP COMM STATUS:                   OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  SPIP COMM STATUS:                   FAIL (%d)\n", s );
  }
  else
  {
    printf( "  SPIP COMM STATUS:                   UNKNOWN (%d)\n", s );
  }
  ...
  s = L2_pmd->sgnl_prcsr_command_stat;
  if( s == 0 )
  {
    printf( "  SIGNAL PROCESSOR COMMAND STATUS:    OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  SIGNAL PROCESSOR COMMAND STATUS:    FAIL (%d)\n", s );
  }
  else
  {
    printf( "  SIGNAL PROCESSOR COMMAND STATUS:    UNKNOWN (%d)\n", s );
  }
  s = L2_pmd->ame_comm_stat;
  if( s == 0 )
  {
    printf( "  AME COMMUNICATIONS STATUS:          OK (%d)\n", s );
  }
  else if( s == 1 )
  {
    printf( "  AME COMMUNICATIONS STATUS:          FAIL (%d)\n", s );
  }
  else
  {
    printf( "  AME COMMUNICATIONS STATUS:          UNKNOWN (%d)\n", s );
  }
  ...
  us = L2_pmd->interpanel_link_status;
  if( us == 0 )
  {
    printf( "  INTERPANEL LINK STATUS:             OK (%d)\n", us );
  }
  else if( us == 1 )
  {
    printf( "  INTERPANEL LINK STATUS:             FAIL (%d)\n", us );
  }
  else if( us == 2 )
  {
    printf( "  INTERPANEL LINK STATUS:             N/A (SINGLE CHANNEL SYSTEM) (%d)\n", us );
  }
  else
  {
    printf( "  INTERPANEL LINK STATUS:             UNKNOWN (%d)\n", us );
  }
/*
  printf( "  SPARE-468:                          %d\n", L2_pmd->spare468[0] );
*/
  printf( "  PERFORMANCE CHECK TIME:             %ld sec\n", (long) L2_pmd->perf_check_time );
...
int libL2_print_rda_status_msg( int i )
{
  ...
  printf( "  DATE:       %05d (%s)\n", L2_status_msgs[i].msg_hdr.julian_date, 
          Convert_date( L2_status_msgs[i].msg_hdr.julian_date ) );
  printf( "  TIME:       %05d (%s)\n", L2_status_msgs[i].msg_hdr.milliseconds, 
	  Convert_milliseconds( L2_status_msgs[i].msg_hdr.milliseconds ) );
  ...
  us = (L2_status_msgs[i].cmd & 0x3e) >> 1;
  printf( "  CMD SEGMENTS:     %x\n", us );

  us = L2_status_msgs[i].avset & RS_AVSET_MASK;
  if( us == AVSET_ENABLED )
  {
  ...
  us = (L2_status_msgs[i].avset & RS_EBC_MASK);
  if( us != 0 )
  {
    printf( "  EBC:            ENABLED (%x)\n", us );
  }
  else 
  {
    printf( "  EBC:            DISABLED (%x)\n", us );
  }

  us = (L2_status_msgs[i].avset & RS_RDA_LOG_MASK);
  if( us != 0 )
  {
    printf( "  RDA LOG DATA:   ENABLED (%x)\n", us );
  }
  else 
  {
    printf( "  RDA LOG DATA:   DISABLED (%x)\n", us );
  }

  us = (L2_status_msgs[i].avset & RS_LEVEL1_MASK);
  if( us != 0 )
  {
    printf( "  LEVEL1 RECORD:  ON (%x)\n", us );
  }
  else 
  {
    printf( "  LEVEL1 RECORD:  OFF (%x)\n", us );
  }
  ...
  us = L2_status_msgs[i].sig_proc_states;
  printf( "  SIG PROC STATE: %x\n", us );

  us = L2_status_msgs[i].ack_vcp_num;
  printf( "  ACK VCP NUM:    %d\n", us );

  us = L2_status_msgs[i].version;
  printf( "  VERSION:        %d\n", us );
...
{
  printf( "SIZE:               %d HALFWORDS (%zu BYTES)\n", m->size, m->size*sizeof(short) );
