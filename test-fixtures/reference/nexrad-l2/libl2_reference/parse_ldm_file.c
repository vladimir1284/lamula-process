 ...
 * $Author: steves $
 * $Locker:  $
 * $Date: 2023/08/15 20:54:39 $
 * $Id: parse_ldm_file.c,v 1.18 2023/08/15 20:54:39 steves Exp $
 * $Revision: 1.18 $
 * $State: Exp $
 */
...
#define RDA_PRF_TABLES_TAG      "rda_prf_tables"
...
static int   Print_rda_prf_tables_flag = NO;
...
static void  Print_rda_prf_tables( char *);
...
static void Print_usage( char *av0 )
{
  ...
  fprintf( stderr, "        %s - RDA PRF Tables data\n", RDA_PRF_TABLES_TAG );
...
static int Validate_print_arg( char *arg )
{
  ...
  else if( strcmp( arg, RDA_PRF_TABLES_TAG ) == 0){Print_rda_prf_tables_flag = YES; }
...
static int Read_meta_data()
{
  ...
  while( buf_offset < Uncompressed_length )
  {
    ...
    {
     LevelII_stats_debug( "MSG 32: SIZE: %d INDEX: %d SEGMENT: %d OF %d",
                   msg_size, msg32_index, segment_number, number_of_segments );
      memcpy( (char *)(msg32buf+msg32_index),
              (char *)msg_header+copy_offset, copy_size );
      msg32_index += copy_size;
      msg_header = (RDA_RPG_message_header_t *)msg32buf;
  ...
  LevelII_stats_debug( "MSG SIZES 32: %d 18: %d 15: %d 13: %d 5: %d 3: %d 2: %d",
  ...
  if( (has_prf) && (Debug_mode || Print_rda_prf_tables_flag) )
  {
    Print_rda_prf_tables( (char *)msg32buf );
  }
  if( Debug_mode || Print_rda_clutter_flag )
  {
    Print_rda_clutter( (char *)msg15buf );
  }
  if( (has_bypass) && (Debug_mode || Print_rda_bypass_flag) )
  {
...
static void Print_rda_adapt( char *buf )
{
  ...
  LevelII_stats_out( "  LOWER PRE-LIMIT SWITCH:  %f", rda_adapt.lower_pre_limit );
  LevelII_stats_out( "  LATENCY OF DCU AZ MEASURE:  %f", rda_adapt.az_lat );
  LevelII_stats_out( "  UPPER PRE_LIMIT SWITCH:  %f", rda_adapt.upper_pre_limit );
  ...
  LevelII_stats_out( "  SPIP 5V PS TOLERANCE:  %f", rda_adapt.spip_5V_reg_lim );
  LevelII_stats_out( "  SPIP 15V PS TOLERANCE:  %f", rda_adapt.spip_15V_reg_lim );
  ...
  LevelII_stats_out( "  A_RPG_LINK_TYPE:  %d", rda_adapt.a_rpg_link_type );
  ...
  LevelII_stats_out( "  INITIAL SYSTEM DIFFERENTIAL PHASE:  %f", rda_adapt.init_phi_dp );
  LevelII_stats_out( "  NORMALIZED INITIAL SYSTEM DIFFERENTIAL PHASE: %f", rda_adapt.norm_init_phi_dp );
  ...
  LevelII_stats_out( "  AME HORIZ TEST SIG PWR:  %f", rda_adapt.h_ts_cw );
  LevelII_stats_out( "  STALO PWR AT A1J2:  %f", rda_adapt.ts_stalo );
  LevelII_stats_out( "  AME NOISE SOURCE HORIZONTAL EXCESS NOISE RATIO:  %f", rda_adapt.ame_h_noise_enr );
...
*/
  ...
  LevelII_stats_out( "  AZIMUTH MOTOR POSITIVE SUSTAINING DRIVE:  %f", rda_adapt.az_pos_sustain_drive );
  LevelII_stats_out( "  AZIMUTH MOTOR NEGATIVE SUSTAINING DRIVE:  %f", rda_adapt.az_neg_sustain_drive );
  LevelII_stats_out( "  INITIAL ESTIMATE AZIMUTH POSITIVE DRIVE SLOPE:  %f", rda_adapt.az_nom_pos_drive_slope );
  LevelII_stats_out( "  INITIAL ESTIMATE AZIMUTH NEGATIVE DRIVE SLOPE:  %f", rda_adapt.az_nom_neg_drive_slope );
  LevelII_stats_out( "  AZIMUTH VELOCITY FEEDBACK SLOPE:  %f", rda_adapt.az_feedback_slope );
  LevelII_stats_out( "  ELEVATION MOTOR POSITIVE SUSTAINING DRIVE:  %f", rda_adapt.el_pos_sustain_drive );
  LevelII_stats_out( "  ELEVATION MOTOR NEGATIVE SUSTAINING DRIVE:  %f", rda_adapt.el_neg_sustain_drive );
  LevelII_stats_out( "  INITIAL ESTIMATE ELEVATION POSITIVE DRIVE SLOPE:  %f", rda_adapt.el_nom_pos_drive_slope );
  LevelII_stats_out( "  INITIAL ESTIMATE ELEVATION NEGATIVE DRIVE SLOPE:  %f", rda_adapt.el_nom_neg_drive_slope );
  LevelII_stats_out( "  ELEVATION VELOCITY FEEDBACK SLOPE:  %f", rda_adapt.el_feedback_slope );
  LevelII_stats_out( "  SLOPE FOR FIRST INTERVAL OF EL POSITION FEEDBACK CURVE:  %f", rda_adapt.el_first_slope );
  LevelII_stats_out( "  SLOPE FOR SECOND INTERVAL OF EL POSITION FEEDBACK CURVE:  %f", rda_adapt.el_second_slope );
  LevelII_stats_out( "  SLOPE FOR THIRD INTERVAL OF EL POSITION FEEDBACK CURVE:  %f", rda_adapt.el_third_slope );
  LevelII_stats_out( "  NEUTRAL DROOP ANGLE:  %f", rda_adapt.el_droop_pos );
  LevelII_stats_out( "  90 DEG OFF NEUTRAL DRIVE:  %f", rda_adapt.el_off_neutral_drive );
  ...
  LevelII_stats_out( "  HORIZ TR LIMITER FAILURE LIMIT:  %f", rda_adapt.h_tr_limit_fail_lim );
  ...
  LevelII_stats_out( "  LOW EQUIP SHELTER TEMP WARNING LIMIT:  %f", rda_adapt.a_min_shelter_temp_warn );
  LevelII_stats_out( "  POWER METER ZERO BIAS VOLTAGE:  %f", rda_adapt.pwr_meter_zero );
...
static void Print_rda_prf_tables( char *rda_data )
{
  int i, j = 0;
  int *fword = NULL;
  short n_prfs, n_waveforms, waveform_code, *sword = NULL;
  float prf = 0.0f, prt = 0.0f, unam_rng_kilo = 0.0f, unam_rng_naut = 0.0f,nyq_velo = 0.0f;
  sword = (short *) ((char *) rda_data + sizeof(RDA_RPG_message_header_t));
  n_waveforms = *sword;
  sword += 2;
  ...
  for( i = 0; i < n_waveforms; i++ ){
   ...
   if(n_prfs > 0){
    if( (waveform_code == VCP_WAVEFORM_CS)
                ||
      (waveform_code == VCP_WAVEFORM_CD)
                ||
      (waveform_code == VCP_WAVEFORM_STP) ){

       fword = (int *) sword;

       //print out waveform name/code, "Contiguous Surveillance", "Contiguous Doppler" or "Staggered PRT"

       if(waveform_code == VCP_WAVEFORM_CS){
          waveform_title = "Contiguous Surveillance";
        } else if (waveform_code == VCP_WAVEFORM_CD) {
	  waveform_title = "Contiguous Doppler";
        } else if (waveform_code == VCP_WAVEFORM_STP) {
          waveform_title = "Staggered PRT";
        }

       printf( "  PRF WAVEFORM CODE:       %s \n", waveform_title);

       for(j = 0; j < n_prfs; j++){
        prf = (float) *fword / 1000.0;
        //print out PRF Data, comes out as digits so attached words to make sure that the info is actually readable.

        prt = (1/prf) * pow(10, 6);
        unam_rng_kilo = (.3) * (prt / 2);
        unam_rng_naut = unam_rng_kilo / 1.852;
        nyq_velo = pow(10, 5) / (4 * prt);

        printf( "   PRF#      PPS     PRT      Rmax(km)    Rmax(nm)    Nyquist(m/s) \n" );
        printf( "   %d:      %4.2f   %4.2f     %4.2f       %4.2f        %4.2f \n",
		       	j, prf, prt, unam_rng_kilo, unam_rng_naut, nyq_velo);
...
static void Print_rda_vcp( char *buf )
{
  ...
  LevelII_stats_out( "  VERSION:        %d", elev_data.version );
...
static void Print_rda_pmd( char *buf )
{
  ...
  LevelII_stats_out( "  ROUTE TO RPG:                       %d", pmd.route_to_RPG );
  ...
  LevelII_stats_out( "  LAN SWITCH CPU UTIL PCT:            %d", pmd.lan_switch_cpu_util );
  LevelII_stats_out( "  LAN SWITCH MEM UTIL PCT:            %d", pmd.lan_switch_mem_util );
  LevelII_stats_out( "  IFDR CHASSIS TEMP:                  %d", pmd.ifdr_chassis_temp );
  LevelII_stats_out( "  IFDR FPGA TEMP:                     %d", pmd.ifdr_fpga_temp );
  LevelII_stats_out( "  NTP_STATUS:                         %d", pmd.ntp_status );
  ...
  LevelII_stats_out( "  RCP STATUS:            %d", pmd.rcp_status );
  LevelII_stats_out( "  RCP STRING:            %s", pmd.rcp_string );
  LevelII_stats_out( "  SPIP POWER BUUTONS:    %d", pmd.spip_pwr_buttons );
  LevelII_stats_out( "  MASTER PWR ADM LOAD:   %f", pmd.master_pwr_admin_load );
  LevelII_stats_out( "  EXP PWR ADM LOAD:      %f", pmd.exp_pwr_admin_load );
  ...
  LevelII_stats_out( "  XMTR/SPIP INTFCE: %d", pmd.xmtr_spip_interface );
  ...
  LevelII_stats_out( "  XMTR PWR MTR ZERO:   %f", pmd.xmtr_pwr_mtr_zero );
  ...
  LevelII_stats_out( "  SPIP +15v PS:            %f", pmd.spip_p_15v_ps );
  LevelII_stats_out( "  SPIP -15v PS:            %f", pmd.spip_n_15v_ps );
  LevelII_stats_out( "  SPIP +28v PS:            %d", pmd.spip_p_28v_ps );
  LevelII_stats_out( "  GENERATOR FUEL LEVEL:    %d", pmd.cnvrtd_gnrtr_fuel_lvl );
  LevelII_stats_out( "PEDESTAL" );
  LevelII_stats_out( "  ELEV +DEAL LIMIT:          %d", pmd.elev_p_dead_lmt );
  ...
  LevelII_stats_out( "  ELEV HOUSING 5V PS:        %d", pmd.elev_housing_5v_ps );
  LevelII_stats_out( "  ELEV -DEAD LIMIT:          %d", pmd.elev_n_dead_lmt );
  ...
  LevelII_stats_out( "  AZ HOUSING 5V PS:          %d", pmd.azmth_housing_5v_ps );
  ...
  LevelII_stats_out( "  PRF SET READ STATUS:               %d", pmd.prf_set_rd_stat );
  ...
  LevelII_stats_out( "  SPIP COMM STATUS:      %d", pmd.spip_comm_stat );
  LevelII_stats_out( "  HCI COMM STATUS:       %d", pmd.hci_comm_stat );
  LevelII_stats_out( "  SIG PROC COMM STATUS:  %d", pmd.sgnl_prcsr_command_stat );
  ...
  LevelII_stats_out( "  INTERPANEL LINK STATUS: %u", pmd.interpanel_link_status );
  LevelII_stats_out( "  PERF_CHECK_TIME:       %u", pmd.perf_check_time );
}
...
static void Print_rda_status( char *buf )
{
  ...
  LevelII_stats_out( "  AVSET:          %d", rda_status_msg->avset & RS_AVSET_MASK );
  LevelII_stats_out( "  EBC:            %d", rda_status_msg->avset & RS_EBC_MASK );
...
static void Print_RRAD( Generic_rad_t *rrad )
{
  ...
  LevelII_stats_out( "FLAGS:       %d", rrad->flags );
}
