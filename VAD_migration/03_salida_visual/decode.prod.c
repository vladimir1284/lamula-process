 ...
 * $Author: cwhite $
 * $Locker:  $
 * $Date: 2023/07/17 19:46:21 $
 * $Id: decode.prod.c,v 1.63 2023/07/17 19:46:21 cwhite Exp $
 * $Revision: 1.63 $
 * $State: Exp $
 */
...
{
   int product_offset, full;

   int block1_size, block1_num_layers, layer1_size = 0;
   ...
   if( product_type == 0xAF1F ) { 
      ...
      decode_radial_rle( &product_data[ product_offset ] ); 
      ...
      decode_raster_rle( &product_data[ product_offset ] );
   ...
   else if (product_type == 27)
   {
      ...
      decode_hires_raster( &product_data[ product_offset ] );
   }
...
void decode_meso( short *product_data, short product_offset, 
   ...
   int last_index, number_symbols, cnt, i;
   short packet_type, packet_size;
   static short four_km = 4;
...
void decode_tabular_block( short *product_data,
   ...
   int block_offset, char_this_line, i;
   int lines_this_page;
...
void product_dependent( short *product_data ){
   ...
   int product_code, int_range, int_azimuth;
