...
/* $Author: steves $ */
/* $Locker:  $ */
/* $Date: 2016/03/03 18:53:45 $ */
/* $Id: vwindpro_alg.c,v 1.10 2016/03/03 18:53:45 steves Exp $ */
/* $Revision: 1.10 $ */
/* $State: Exp $ */
...
int A317a2_vad_buffer_control(){
    ...
    int envstat, ref_flag, vel_flag, wid_flag;
	    ...
	    for( ht = 0; ht < MAX_VAD_HTS; ++ht ){
            ...
            for( ht = 0; ht < MAX_VAD_HTS; ++ht ){
                ...
                if( A317vs->elcn[celv][ht] == bhd->rpg_elev_ind ){
                    ...
                    /* Set the ambiguous range based save areas to initial value */
                    for( i = 0; i < MAX_NUM_AMBRNG; ++i )
                        A317vs->ambig_range[ht][celv][i] = 0;
